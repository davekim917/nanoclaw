...
async function spawnContainer(
  ...
  // Read-only, through the seam: a session with no mailbox has no fence, and a
  // spawn must never provision one here (invariant I-4). The session is closed
  // before the read's verdict is acted on, so nothing downstream of this spawn
  // is inside a mailbox session for this key (invariant I-3).
  const repositoryFence = await withExistingNanoclawSession(session.agent_group_id, session.id, (mailbox) =>
