...
export function composeGroupClaudeMd(group: AgentGroup, provider: string): void {
  ...
  // siblings. Flattened rather than concatenated so any `@`-include expands
  // instead of reaching the model as literal text.
  const localBody = flattenClaudeMd(localFile, { containerToHost }).trim();
  const agentsHeader =
