...
- **The barrier's answer.** On every `lanes` and `synthesis` fire the
  ...
  **A refusal that appears AFTER the brief was acknowledged re-offers the
  ...
  while the `.ack` is absent (`smoke-campaign-controller.py:2334-2336`), so that
  ...
  `briefedRefusal` at `:1314-1315`, carried forward at `:2327-2328`, compared by
  `_reoffer_on_new_refusal` at `:2468`) — and
  ...
  measured from the obligation's first record (`smoke-campaign-controller.py:2345`), so an owner that keeps producing
