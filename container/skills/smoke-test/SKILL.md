...
- **The barrier's answer.** On every `lanes` and `synthesis` fire the
  ...
  (`smoke-campaign-controller.py:2110` `publish_barrier`, whose effect writes at
  `:1523-1524` and unlinks on `doc is None` at `:1509-1517`), and every
  barrier-backed brief names that file (`:1544-1567`). `invalid[]` is artifact
  ...
  **A refusal that appears AFTER the brief was acknowledged re-offers the
  ...
  while the `.ack` is absent (`smoke-campaign-controller.py:2682-2684`), so that
  ...
  `briefedRefusal` at `:1462-1463`, carried forward at `:2675-2676`, compared by
  `_reoffer_on_new_refusal` at `:2816`) — and
  ...
  measured from the obligation's first record (`smoke-campaign-controller.py:2693`), so an owner that keeps producing
  ...
  **A refusing barrier wakes the owner on BOTH phases.** By the time a run
  ...
  without a wake (it now wakes at `smoke-campaign-controller.py:3381-3383`),
  ...
  branch declined to create (`_maybe_synthesis_overdue_blocked`, `:3425`,
  reading that obligation at `:3431-3432`), so it could not fire either.
