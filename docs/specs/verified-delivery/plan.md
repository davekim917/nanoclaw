...
**Generator output is harness-agnostic.** The files the generator emits into a project repo
describe that project only: no harness paths, no other workgroups, no agent names, no
examples drawn from other projects, and no credentials. Test credentials come from the
project's own secret store, referenced by name. A project repo must be able to read as if no
harness existed.
...
| 0a | Before any kit code lands in the public bootstrap plugin repo, give that repo the same public-boundary scan this repo runs, in pre-commit and in CI. It is public, and today its hooks check only plugin parity | a staged private identifier fails that repo's commit hook and its CI |
...
Recorded 2026-09-26. The first is the operator's; the rest are recommendations the operator
deferred to, and any of them can be overruled.

1. **First product instance:** chosen — a web app with PR previews (sequence step 2).
2. **Gate scope: "map it or declare it", from day one.**
   - A PR that changes behavior carries a receipt for the affected feature.
   - If that feature is not mapped yet, the PR adds its feature file in the same PR, so the
     map grows with the work instead of in a separate backfill project.
   - A PR with no user-visible effect declares so, with a reason. Review challenges the
     declaration, and the cross-repo consumer map decides it: "no frontend files changed"
     is not a valid reason on its own.
   - Why not the alternatives: gating every PR from day one blocks everything, because
     nothing is mapped yet; gating only already-mapped features lets unmapped features stay
     unmapped forever, and they are exactly where the misses happen.
3. **Routine proof moves to builder receipts.** Agents merge PRs below the high-risk tier on
   receipt plus automated review. The independent challenger runs on release freezes and
   high-risk changes, not on every routine PR. Target: a receipt in under 10 minutes,
   including preview readiness. Measure readiness first; if the preview takes longer than
   that to come up, fix that before anything else.
4. **Native-only changes split merge from release.**
   - Merge on source proof (unit and component tests, CI), with the receipt marked
     "source-verified, device-unverified".
   - Release is gated on a manual test packet generated by the run tool: build ID, test
     account, exact steps, expected result, what to record. The result is recorded against
     that build.
   - Revisit device automation only if manual testing becomes the bottleneck.

**Caveat.** Receipts alone do not reproduce the talk's throughput. The other half is a
locked-down codebase: per-repo lint rules and structure grown from repeat mistakes, so
low-context agents do the right thing by default. Both halves are required.
...
- 2026-09-26: first product instance chosen; gate scope, routine-proof routing and the native
  split recorded as recommended defaults (see Decisions). No build started.
