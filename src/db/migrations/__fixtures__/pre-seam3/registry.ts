/**
 * FROZEN SNAPSHOT — the central migration registry exactly as it stood on
 * `origin/main` before seam 3 PR 2 reconciled the ledger with upstream.
 *
 * Production code never imports this file. Its only consumer is
 * `src/db/migrations/registry.test.ts`, which uses it to build a database that
 * looks like a LIVE install (one that already ran the old ledger) and then
 * proves the reconciled registry converges on the fresh-install schema.
 *
 * The three files this PR replaced are frozen alongside it — `019-wiring-threads.ts`
 * (pre-`sqliteOnly` copy), `065-container-config-timezone.ts` and
 * `066-approvals-instance.ts`, the two files whose upstream equivalents (020 and
 * 023) now carry the same `name`. Every other entry is imported from the live
 * directory: those files are unchanged by this PR, and the snapshot that matters
 * is the LIST — which names ran, in which order — not a second copy of 130 files.
 *
 * Do not add entries here. A future migration belongs in the live registry; this
 * list is what live installs have already applied.
 */
...
/** The pre-reconciliation `migrations` array, byte-for-byte in order. */
export const preSeam3Migrations: Migration[] = [
  migration001,
  migration002,
