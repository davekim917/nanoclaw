      ...
      <div className="nc-task-detail">
        ...
        <ul className="nc-transcript">
          {data.transcript.map((e) => (
            <TranscriptRow key={`${e.direction}-${e.seq}`} entry={e} />
          ))}
        </ul>
      </div>
    </div>
...
function TranscriptRow({ entry }: { entry: SessionTranscriptEntry }) {
  ...
  const text = entry.text.length > 600 ? entry.text.slice(0, 600) + '…' : entry.text;
  return (
    <li className={`nc-transcript-row ${isInbound ? 'inbound' : 'outbound'}`}>
      ...
      <div className="nc-transcript-text">{text}</div>
    </li>
  );
}
