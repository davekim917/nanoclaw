...
export const MANAGED_GIT_OPENCODE_PLUGIN_PATH = '/app/src/managed-git-guard-opencode.ts';
...
export type ManagedGitGuardDecision = { action: 'allow' } | { action: 'deny'; operation: string; reason: string };

function managedRepositoryWorkspaceConfigured(env: NodeJS.ProcessEnv): boolean {
  return Boolean(env.NANOCLAW_HOST_DATA_DIR || env.NANOCLAW_HOST_TOPIC_WORKTREES_DIR);
}
...
function commandStart(words: string[]): number {
  ...
  while (index < words.length) {
    ...
    if (executable === 'env' || executable === 'sudo' || executable === 'doas') {
      index = skipWrapperOptions(words, index + 1);
      continue;
    }
    break;
...
function gitSubcommand(
  ...
  while (index < words.length) {
    ...
    if (option === '-c' && value) {
      const match = value.match(/^alias\.([^=]+)=(.*)$/s);
...
function protectedGitOperation(words: string[], depth = 0): string | null {
  ...
  if (parsed.command === 'worktree') {
    ...
    if (action && PROTECTED_WORKTREE_ACTIONS.has(action)) {
      return `git worktree ${action}`;
    }
  }
...
export function evaluateManagedGitCommand(
  command: string,
  env: NodeJS.ProcessEnv = process.env,
): ManagedGitGuardDecision {
  if (!command || !managedRepositoryWorkspaceConfigured(env)) return { action: 'allow' };
...
export function createManagedGitMaintenanceHook(): HookCallback {
  return async (input) => {
    const pre = input as PreToolUseHookInput;
    const command = (pre.tool_input as { command?: string } | undefined)?.command;
    if (!command) return {};
    const decision = evaluateManagedGitCommand(command);
...
export function guardManagedGitCommandOrThrow(command: string): void {
  const decision = evaluateManagedGitCommand(command);
