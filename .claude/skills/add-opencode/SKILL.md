...
If you set nothing, the group inherits the code default `opencode/gemini-3.7-flash` (effort `high`). Effort accepts `low|medium|high` (opencode's portable set). The `provider/model-id` shapes in the examples below are still the right FORMAT for `--model` — just pass them to `ncl`, not `.env`.
