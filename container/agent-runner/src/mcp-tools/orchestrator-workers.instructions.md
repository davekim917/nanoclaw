...
- Invoke `worker-codex` with `run_in_background: true`. This backgrounds only the Claude worker at the parent layer; the worker keeps its `codex exec` Bash call in the foreground so lifecycle, cancellation, and results stay attached.
