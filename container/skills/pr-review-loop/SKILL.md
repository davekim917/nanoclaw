...
**A head that moved only by bookkeeping needs a quick re-receipt, not a new review or a ci-full rerun.** A receipt covers exactly one head, so any move needs a new one. The quick path applies only when the new head is the approved head with the base merged in, and the only other changes are a regenerated `src/upstream-ratchet.json` and/or `docs/review-notes/` fragments. Prove it on the new head with an exact check of the merge result. `range-diff` won't do, because it never looks inside the merge commit. Run the check from a checkout of the new head:
...
( set -o pipefail
  ...
  [ "$rc" -le 1 ] || { echo "QUICK PATH REFUSED: merge-tree exit $rc"; exit 1; }
  T=$(printf '%s\n' "$out" | head -1)
  git cat-file -e "$T^{tree}" 2>/dev/null || { echo "QUICK PATH REFUSED: no merge tree"; exit 1; }
  changed=$(git diff --name-only "$T" "$NEW") || { echo "QUICK PATH REFUSED: diff failed"; exit 1; }
  extra=$(printf '%s\n' "$changed" | grep -v -E '^(src/upstream-ratchet\.json|docs/review-notes/[^/]+\.md)?$')
  [ -z "$extra" ] || { echo "QUICK PATH REFUSED: $extra"; exit 1; }
  echo "QUICK PATH OK" )
flock <scratchpad>/vitest.lock ionice -c3 nice -n 10 node_modules/.bin/vitest run src/upstream-ratchet.test.ts scripts/review-notes.test.ts --maxWorkers=2
```

`T` is the tree git's automatic merge of the approved head and the base produces. Only `QUICK PATH OK`, and both tests passing, let you take the quick path. Any `REFUSED` line or a failing test means a full review, and ci-full if code changed. That includes a hand-resolved conflict in any other file, which differs from `T` and so is listed. Paste both outputs into the receipt body, and make sure the PR's `CI` is green on the new head. Don't re-run ci-full for a quick-path move: the gate requires only the workflows in `CODEX_REVIEW_REQUIRED_WORKFLOWS`, default `CI` (the `ci_status` comment in `scripts/codex-review.sh`). The two tests above are what catch a stale hash or a bad citation, so they are run here explicitly rather than assumed to be in CI.
