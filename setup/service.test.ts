...
describe('systemd unit generation', () => {
  it('user unit uses default.target', () => {
    const unit = renderSystemdUnit('/usr/bin/node', '/home/user/nanoclaw', '/home/user', false);
    expect(unit).toContain('WantedBy=default.target');
  });

  it('system unit uses multi-user.target', () => {
    const unit = renderSystemdUnit('/usr/bin/node', '/home/user/nanoclaw', '/home/user', true);
  ...
  it('sets correct ExecStart', () => {
    const unit = renderSystemdUnit('/usr/bin/node', '/srv/nanoclaw', '/home/user', false);
    expect(unit).toContain('ExecStart=/usr/bin/node /srv/nanoclaw/dist/index.js');
  });

  it('prepends the selected nodeenv directory and deduplicates ~/.local/bin', () => {
    const unit = renderSystemdUnit('/home/user/.local/bin/node', '/home/user/nanoclaw', '/home/user', false);
    const pathLine = unit.split('\n').find((line) => line.startsWith('Environment=PATH='));
    expect(pathLine).toBe('Environment=PATH=/home/user/.local/bin:/usr/local/bin:/usr/bin:/bin');
  });
});
