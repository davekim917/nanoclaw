...
- **The barrier's answer.** On every `lanes` and `synthesis` fire the
  ...
  (`smoke-campaign-controller.py:1850` `publish_barrier`, whose effect writes at
  `:1395-1396` and unlinks on `doc is None` at `:1381-1389`), and every
  barrier-backed brief names that file (`:1416-1439`). `invalid[]` is artifact
  ...
  **A refusal that appears AFTER the brief was acknowledged re-offers the
  ...
  while the `.ack` is absent (`smoke-campaign-controller.py:2395-2397`), so that
  ...
  (`refusal_digest`, `smoke-campaign-controller.py:243`; recorded as
  `briefedRefusal` at `:1334-1335`, carried forward at `:2388-2389`, compared by
  `_reoffer_on_new_refusal` at `:2529`) — and
  ...
  measured from the obligation's first record (`smoke-campaign-controller.py:2406`), so an owner that keeps producing
  ...
  **A refusing barrier wakes the owner on BOTH phases.** By the time a run
  ...
  without a wake (it now wakes at `smoke-campaign-controller.py:3024-3026`),
...
- **A re-minted owner token.** `poll` mints a fresh coordinator owner token on
  every same-SHA recovery (`smoke-pr-gate.sh:5322`), which is how a coordinator
