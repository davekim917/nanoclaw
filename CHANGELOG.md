...
- [BREAKING] **The `opus` alias now means Opus, and an unpinned Claude group runs it at `medium`.** `ANTHROPIC_DEFAULT_OPUS_MODEL` — the SDK short-circuit deciding what the bare word `opus` resolves to — carried each group's own resolved model rather than an Opus id, so "opus" meant "whatever this group runs". When the unpinned Claude default moved to Sonnet 5 on 2026-09-14, the `"model": "opus"` pin that group-init writes into **every** group's `.claude-shared/settings.json` started resolving to `claude-sonnet-5` (measured live in a running container), and so did every `model: opus` subagent. The three `ANTHROPIC_DEFAULT_<FAMILY>_MODEL` vars are now install-wide constants in every group, and the group's own model travels as `NANOCLAW_CLAUDE_MODEL`. Alongside it, two defaults: an unpinned Claude group resolves to `claude-opus-5[1m]` (was `claude-sonnet-5`), and Opus's per-family default effort moves `high` → `medium`, since that default is now the fleet's baseline burn rather than a deliberate escalation's. **This changes production behaviour on deploy** — every unpinned Claude group's conversations move from Sonnet 5/`xhigh` to Opus 5 [1m]/`medium` on its next spawn, at Opus pricing. Groups and channels with an explicit model pin keep it. **Migration — pin BEFORE you deploy:** [docs/claude-default-model.md](docs/claude-default-model.md) (detect / why / fix / verify / rollback); pins are data and need no deploy, the defaults are code and need one.
