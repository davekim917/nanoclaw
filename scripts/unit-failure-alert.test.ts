...
function fixture(): { root: string; outbox: string; bin: string } {
  ...
  return { root, outbox, bin };
}

function run(outbox: string, bin: string) {
  return spawnSync('bash', [SCRIPT, UNIT], {
    encoding: 'utf8',
    env: { ...process.env, PATH: `${bin}:${process.env.PATH}`, UNIT_ALERT_OUTBOX: outbox },
  });
}

describe('unit-failure-alert.sh', () => {
  it('queues the alert as a regular .md file and leaves no temp file behind', () => {
    const { outbox, bin } = fixture();
    const result = run(outbox, bin);
