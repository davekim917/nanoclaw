...
export const spawnProgress: McpToolDefinition = {
  ...
  async handler(args) {
    ...
    return emitSpawnAction('spawn_progress', args, { message }, `Progress reported: ${message}`, ` — ${message}`);
  },
};
...
export const spawnComplete: McpToolDefinition = {
  ...
  async handler(args) {
    ...
    return emitSpawnAction('spawn_complete', args, { summary }, 'Task completed. Summary sent to orchestrator.');
  },
};
...
export const spawnRequestSteer: McpToolDefinition = {
  tool: {
