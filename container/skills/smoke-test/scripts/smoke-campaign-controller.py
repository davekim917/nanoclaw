...
# Open issues listed for title dedup; a listing that reaches it is incomplete
# and files nothing (EffectLayer._gh).
ISSUE_DEDUP_LIMIT = 5000
...
class EffectLayer:
    ...
    def _gh(self, effect):
        ...
        if slot.startswith("issue:"):
            ...
            repo_labels = self._labels_of(repo)
            # SI: dedup against open smoke-finding issues by normalized title,
            # over the COMPLETE open set. No `--label` and no `--search`: either
            # sends gh through the search API (pkg/cmd/issue/list/list.go:235-244
            # at gh v2.101.0), which is index-backed, capped at 1000 (http.go:188)
            # and query-length-limited. Unfiltered, gh pages the GraphQL listing
            # (http.go:33) to --limit, so the scope is applied here from each
            ...
            if len(open_issues) >= ISSUE_DEDUP_LIMIT:
                ...
                return {"outcome": "unknown",
                        "error": "open-issue listing hit its {} cap; not filing".format(ISSUE_DEDUP_LIMIT)}
            scoped = repo_labels is None or "smoke-finding" in {n.lower() for n in repo_labels}
            norm = normalize_title(title)
            for issue in open_issues:
                if not isinstance(issue, dict) or normalize_title(issue.get("title")) != norm:
                    continue
                names = {str(x.get("name")).lower() for x in issue.get("labels") or [] if isinstance(x, dict)}
                if not scoped or "smoke-finding" in names:
                    return {"outcome": "done", "via": "dedup", "url": issue.get("url"),
                            "duplicateOf": issue.get("number")}
            labels, mapped, dropped = resolve_labels(labels, repo_labels)
            name = key[:16] + "-issue.md"
            if dropped:
                note = "Labels not on {} and dropped: {}.".format(repo, ", ".join("`{}`".format(x) for x in dropped))
                body += "\n\n" + note
                # The payload is written once per name and a replay keeps it
                # (write_contained_once), so a body that gains this note after
                # an earlier attempt -- a listing that failed, then recovered --
                # needs its own name, or the note never reaches the issue. The
                # marker search above still makes the create land at most once.
                name = "{}-{}-issue.md".format(key[:16], hashlib.sha256(note.encode()).hexdigest()[:8])
            path, _ = write_contained_once(root, self._payload_dir(run_id) + [name], "{}\n{}\n".format(body, marker))
            argv = self.gh + ["issue", "create", "-R", repo, "--title", title, "--body-file", path]
            for label in labels:
                argv += ["--label", label]
            wrote = self._run(argv, 30)
            # Read back the issue the create names: the issues listing can lag
            # a fresh create, and a miss here pops the fire's cached listing.
            # The listing scan stays the fallback -- no URL printed, or the GET
            # failed -- so a create that may have landed is never read as "not
            # created".
            found, err = self._created_issue(repo, wrote, marker), None
            if not found:
                found, err = self._find_marker(api, marker)
    ...
    def _dedup_note(self, run_id):
        ...
        pr-comment in the same fire (:3624-3628); one still unsettled then is not
    ...
    def _created_issue(self, repo, wrote, marker):
        """The URL `gh issue create` printed (its stdout's last line, written
        only once the issue exists), confirmed by a direct GET of that issue
        carrying our marker. None when there is no URL, the GET fails, or the
        body lacks the marker: the caller falls back to the listing scan."""
        lines = [x.strip() for x in str(wrote[1] or "").splitlines() if x.strip()]
        number = re.search(r"/issues/(\d+)$", lines[-1]) if lines else None
        if not number:
            return None
        # Whatever a failed GET prints, only a JSON issue whose body carries
        # our marker is accepted, so any failure falls through to None.
        _, out, _ = self._run(self.gh + ["api", "repos/{}/issues/{}".format(repo, number.group(1))], 20)
        try:
            doc = json.loads(out)
        except ValueError:
            return None
        if not isinstance(doc, dict) or marker not in str(doc.get("body") or ""):
            return None
        return doc.get("html_url") or lines[-1]
    ...
    def _owner_wake(self, effect):
        ...
        # :2738-2748); the renewer reads it the same way
...
def resolve_labels(wanted, repo_labels):
    ...
    create the same way, so it is dropped too, and the title dedup then lists
    that repo's open issues unlabelled (_gh), since the marker bounds only
    one run's retries. repo_labels None = the listing failed:
...
class Controller:
    ...
    def _reoffer_on_new_refusal(self, run_id, step):
        ...
        (:2738-2748). The diagnosis is on disk and nobody is told to read it --
        ...
        (:2749) -- so a run that keeps producing invalid evidence still ends at
    ...
    def step_run(self, run_id):
        ...
        if syn_err == "missing":
            if not syn_barrier.get("ready"):
                ...
                # barrier is ready (:3403), coordinator/preliminary.md exists
                # (:3420) and challenger/disposition.md exists (:3426). What
    ...
    def _terminal_before_root(self, run_id, claim, run, pr):
        ...
        2. An owner's own BLOCKED/HUMAN_DECISION synthesis is taken by the
           ...
           contract's (:2036-2043), so without one it could only be BLOCKED.
    ...
    def _publish_verb(self, run_id, run, verb):
        ...
        calling the gate (this file, :2448-2449), so re-selecting the verb
