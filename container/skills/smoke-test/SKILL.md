...
- **The barrier's answer.** On every `lanes` and `synthesis` fire the
  ...
  (`smoke-campaign-controller.py:2126` `publish_barrier`, whose effect writes at
  `:1539-1540` and unlinks on `doc is None` at `:1525-1533`), and every
  barrier-backed brief names that file (`:1560-1583`). `invalid[]` is artifact
  ...
  **A refusal that appears AFTER the brief was acknowledged re-offers the
  ...
  while the `.ack` is absent (`smoke-campaign-controller.py:2698-2700`), so that
  ...
  `briefedRefusal` at `:1478-1479`, carried forward at `:2691-2692`, compared by
  `_reoffer_on_new_refusal` at `:2832`) — and
  ...
  measured from the obligation's first record (`smoke-campaign-controller.py:2709`), so an owner that keeps producing
  ...
  **A refusing barrier wakes the owner on BOTH phases.** By the time a run
  ...
  without a wake (it now wakes at `smoke-campaign-controller.py:3397-3399`),
  ...
  branch declined to create (`_maybe_synthesis_overdue_blocked`, `:3441`,
  reading that obligation at `:3447-3448`), so it could not fire either.
