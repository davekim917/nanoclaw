...
describe('CodexProvider app-server-only recovery', () => {
  it('replaces an unresponsive app-server, resumes the same thread, and continues without duplicating the prompt', async () => {
...
`,
    ...
    expect(events.some((event) => event.type === 'progress' && event.message?.includes('restarting app-server'))).toBe(
      true,
    );
