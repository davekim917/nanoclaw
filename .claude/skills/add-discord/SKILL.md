...
4. OAuth2 → URL Generator → Scopes: bot; Bot Permissions: View Channel, Send Messages, Send Messages in Threads, Create Public Threads, Create Private Threads, Manage Threads, Embed Links, Attach Files, Read Message History, Add Reactions, Use Slash Commands, and Mention Everyone. Do not grant Manage Channels, Manage Messages, or Manage Webhooks.
...
```nc:operator
Open the invite link — https://discord.com/oauth2/authorize?client_id={{application_id}}&scope=bot&permissions=3378792827444288 — and add the bot to a server you're also in (a personal server works fine); the bot can only DM you once you share a server. If you already invited it while setting up the app, you can skip this.
```
