...
describe('default instance — legacy unprefixed keys (live-install regression arm)', () => {
  it('reads rows written before the namespace dimension existed', async () => {
    ...
    getRawDb().prepare("INSERT INTO chat_sdk_subscriptions (thread_id) VALUES ('T-raw')").run();
  ...
  it('writes raw keys — kv, subscriptions, lists bind the exact input strings', async () => {
    ...
    const kv = getRawDb().prepare('SELECT key FROM chat_sdk_kv').all() as Array<{ key: string }>;
    expect(kv.map((r) => r.key)).toEqual(['k1']);
    const subs = getRawDb().prepare('SELECT thread_id FROM chat_sdk_subscriptions').all() as Array<{
      thread_id: string;
    }>;
    expect(subs.map((r) => r.thread_id)).toEqual(['slack:T1']);
    const lists = getRawDb().prepare('SELECT key FROM chat_sdk_lists').all() as Array<{ key: string }>;
...
describe('namespaced instance — round-trips and raw-key shape', () => {
  it('kv get/set/setIfNotExists/delete round-trip under a prefixed key', async () => {
    ...
    const raw = getRawDb().prepare('SELECT key FROM chat_sdk_kv').all() as Array<{ key: string }>;
  ...
  it('subscribe/isSubscribed/unsubscribe round-trip under a prefixed thread_id', async () => {
    ...
    const raw = getRawDb().prepare('SELECT thread_id FROM chat_sdk_subscriptions').all() as Array<{
      thread_id: string;
    }>;
  ...
  it('lists round-trip under a prefixed key', async () => {
    ...
    const raw = getRawDb().prepare('SELECT DISTINCT key FROM chat_sdk_lists').all() as Array<{ key: string }>;
...
describe('locks under a namespace', () => {
  it('acquire returns the RAW threadId; extend and release hit the prefixed row', async () => {
    ...
    const raw = getRawDb().prepare('SELECT thread_id FROM chat_sdk_locks').all() as Array<{ thread_id: string }>;
    expect(raw.map((r) => r.thread_id)).toEqual(['slack-tester:slack:T1']);

    expect(await state.extendLock(lock!, 10_000)).toBe(true);
    await state.releaseLock(lock!);
    expect(getRawDb().prepare('SELECT COUNT(*) AS c FROM chat_sdk_locks').get()).toEqual({ c: 0 });
  });
