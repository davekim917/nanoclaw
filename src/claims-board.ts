...
export type ClaimState = 'live' | 'expiring' | 'stale';
...
export function readClaims(workgroupId: string, now: number, root: string = claimsBaseDir()): BoardClaim[] {
  ...
  for (const entry of entries) {
    ...
    if (declaresItselfFinished(raw)) continue;
...
const ORDER: ClaimState[] = ['stale', 'expiring', 'live'];
...
export function renderClaims(
  ...
  if (claims.length === 0) return '**Who’s on what** — _nothing claimed right now._';
  ...
  for (const state of ORDER) {
    ...
    for (const c of group) {
      const age =
        c.state === 'live' ? `${duration(c.staleMs)} left` : `${duration(c.staleMs)} past TTL`;
