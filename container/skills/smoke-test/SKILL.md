...
- **The barrier's answer.** On every `lanes` and `synthesis` fire the
  ...
  (`smoke-campaign-controller.py:2115` `publish_barrier`, whose effect writes at
  `:1528-1529` and unlinks on `doc is None` at `:1514-1522`), and every
  barrier-backed brief names that file (`:1549-1572`). `invalid[]` is artifact
  ...
  **A refusal that appears AFTER the brief was acknowledged re-offers the
  ...
  while the `.ack` is absent (`smoke-campaign-controller.py:2687-2689`), so that
  ...
  `briefedRefusal` at `:1467-1468`, carried forward at `:2680-2681`, compared by
  `_reoffer_on_new_refusal` at `:2821`) — and
  ...
  measured from the obligation's first record (`smoke-campaign-controller.py:2698`), so an owner that keeps producing
  ...
  **A refusing barrier wakes the owner on BOTH phases.** By the time a run
  ...
  without a wake (it now wakes at `smoke-campaign-controller.py:3386-3388`),
  ...
  branch declined to create (`_maybe_synthesis_overdue_blocked`, `:3430`,
  reading that obligation at `:3436-3437`), so it could not fire either.
