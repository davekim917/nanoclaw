...
class EffectLayer:
    ...
    def _owner_wake(self, effect):
        ...
        # :2518-2528); the renewer reads it the same way
...
class Controller:
    ...
    def _reoffer_on_new_refusal(self, run_id, step):
        ...
        (:2518-2528). The diagnosis is on disk and nobody is told to read it --
        ...
        (:2529) -- so a run that keeps producing invalid evidence still ends at
    ...
    def _run_dir_expected(self, run_id):
        ...
        :1350 and :1369; owner_step journals it after the effect returns,
        :2503-2513) -- a bare `intent` is deliberately NOT proof, since a fire
    ...
    def step_run(self, run_id):
        ...
        if syn_err == "missing":
            if not syn_barrier.get("ready"):
                ...
                # barrier is ready (:3160), coordinator/preliminary.md exists
                # (:3177) and challenger/disposition.md exists (:3183). What
    ...
    def _terminal_before_root(self, run_id, claim, run, pr):
        ...
        2. An owner's own BLOCKED/HUMAN_DECISION synthesis is taken by the
           ...
           contract's (:1830-1837), so without one it could only be BLOCKED.
    ...
    def _publish_verb(self, run_id, run, verb):
        ...
        calling the gate (this file, :2240-2241), so re-selecting the verb
