...
vi.mock('../../db/channel-ingress-receipts.js', async (importOriginal) => ({
  ...
  pruneChannelIngressReceipts: async () => 0,
}));
...
vi.mock('../../modules/scheduling/live-count.js', async (importOriginal) => ({
  ...(await importOriginal<typeof import('../../modules/scheduling/live-count.js')>()),
  countLiveRowsInSessions: () => ({ count: 0, unreadable: false }),
}));
vi.mock('../../dashboard/api/scheduled-shared.js', async (importOriginal) => ({
  ...(await importOriginal<typeof import('../../dashboard/api/scheduled-shared.js')>()),
  purgeIntentBody: () => undefined,
}));
