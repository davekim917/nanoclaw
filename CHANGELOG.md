...
- **`ncl integrations remove --delete-secret` now drops the bearer from the group's `container.json` before deleting it, in one step.** Removal deliberately left the `onecliSecrets` declaration alone and the docs made editing it a manual step to run *first* — but the refresher re-declares the bearer on every successful refresh (`refreshOne`'s tail), so a sweep tick landing between the hand edit and the command re-declared the name the command was about to delete. `resolveSecretUuids` throws on a declaration it cannot resolve, so the group was then left aborting **every** spawn closed until someone edited the file again. `removeIntegrationLocked` now undeclares and deletes inside the same per-name lock the refresher takes, so the two can no longer interleave. The order is undeclare-then-delete and the undeclare's failure aborts the whole removal with nothing deleted, because the other order is the one that produces the spawn-abort shape; a `container.json` that will not write leaves the integration exactly as it was, and re-running the command is the whole recovery. Both spellings a declaration can take (the secret's name and its vault UUID) are dropped, and no other declaration is touched. Plain `remove` is unchanged: the declaration stays. Host restart only (#929).

- **The review gate no longer lets a never-started re-run hide a genuine red, and `admin=ready` answers fail-closed on three more rules.** During the Actions billing lockout a `CI (host)` success may stand in for a required workflow *GitHub never started* — but the two ways a workflow is re-run both hid an older real failure of the same check from that judgement: `Re-run jobs` keeps the run id and bumps `run_attempt` (so the failed attempt's jobs are only visible under `/attempts/<n>/jobs`), and a fresh trigger makes a second run of the same name (and `ci_verdict` keeps only the newest run per name). `never_started_runs` now requires every *attempt* of a run to be never-started and disqualifies the whole workflow name when any failed run of it on that head is anything else; `required_status_red` no longer excuses a check run whose own job never started unless that run is one `never_started_runs` vouches for. Anything unreadable disqualifies rather than being skipped. Separately, `admin_readiness` is not-ready on a non-author `CHANGES_REQUESTED` review, on `strict_required_status_checks_policy` (GitHub's "require branches to be up to date", which it does not evaluate), and on **any** positive required-approval count — GitHub counts only approvals from accounts with write access and the script cannot tell which have it, so it refuses instead of counting. No repo in this fleet requires an approval today, so that last one refuses nothing that used to pass. Script-only; no deploy (#937, partial).
