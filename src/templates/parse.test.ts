...
describe('parseTemplate', () => {
  ...
  describe('mcp component', () => {
    ...
    // FORK: the credential-header contract is the OneCLI placeholder
    // ("onecli-managed", optionally behind one auth-scheme token), not the
    // Agent Plugins literal "placeholder" — the gateway substitutes the former
    // at the proxy boundary and would ship the latter to the wire verbatim.
    it('accepts a ./-relative command and OneCLI placeholder headers', () => {
      writeManifest();
      writeMcp({
    ...
    it('accepts an auth-scheme-prefixed OneCLI placeholder without a warning', () => {
      writeManifest();
      writeMcp({
        api: {
          type: 'streamable-http',
    ...
    it('warns (but keeps the server) on a secret-shaped key with an unrecognized value', () => {
      writeManifest();
      // Not a declared placeholder and not a known secret format — exactly the
      // value shape the warn-only arm exists for.
      writeMcp({ crm: { type: 'stdio', command: 'server', env: { EXA_API_KEY: 'from-the-vault' } } });
