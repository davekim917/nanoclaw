...
This section is complete on its own. §1–§5 after it are the **2026-09-16** migration (the alias fix, and Opus 5/`high` plus Codex Sol/`high` defaults), kept as history. Only §1's audit script is reused below. Read their expected values as that date's values, not today's.
...
**Detect.** Run the audit script from [§1](#1-detect) as written. Then read its rows with today's values:
- A row moves to Opus 5.5 when its effective model column reads `claude-opus-5-5[1m]` (after this build) or `claude-opus-5[1m]` (before it) and its configured column is `(none)`, `opus`, or a refused value.
- A row moves to `medium` when its effective model is any `claude-opus-*` and its effort column reads `(family default)`.
- Task pins are data. Run `ncl tasks list --json`, then read `model_pin` / `effort_pin` (an `opus` pin moves; a frozen id does not).
