...
- **Declaring the placeholder wires the header; it does not grant the secret.** The gateway can only substitute a credential that is assigned to your agent group. If a newly added remote server returns 401, one likely cause is that the credential is in the vault but not assigned — an operator adds it to `onecliSecrets` in the group's `container.json`, or runs `onecli agents set-secrets`. A 401 can also mean an expired or incorrect secret, a missing gateway rule, or an auth scheme the server doesn't accept, so say the vault-assignment gap as a possibility to the user, not the cause, rather than retrying.
