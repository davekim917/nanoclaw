...
Write only the artifacts named by the phase brief. Preserve prior findings. Before scaffold or barrier commands source the group's gate environment and pass the owner token from the saved wake as `SMOKE_GATE_OWNER`. That environment names the develop pair, so every `smoke-pair-identity.sh` call passes the PR preview ids on its own command line exactly as the brief's pair-identity note prints them; a check against the sourced ids records a drift that blocks the run. A missing active owner or failed phase requires an explicit recovery disposition preserving artifacts, not silent replacement.
