...
- **H-1b** `src/mailbox-seam-ratchet.test.ts` › "no raw session-DB access or passed session handle outside the mailbox modules except the committed allowlist" — grep patterns (a)–(d) of §4.6.2 plus the arity-2 type assertion (e); assert the set of offending files ⊆ `RATCHET.json`; and by PR 7 / R3 assert each half of `RATCHET.json` as an EXACT SET — runner exactly `[]`, host exactly `["src/modules/sweep-scheduled-move/index.ts", "src/storage-manager.ts"]` (the case flips from "subset" to "exact" per half as each half finishes, not to "empty": the host half never reaches zero, and a subset check would accept a finished half growing again).
