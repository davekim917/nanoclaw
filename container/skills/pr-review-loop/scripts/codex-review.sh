...
# head/outcome, never the reviewer, so old (pre-allowlist) receipts still
# parse; see reviewer_model_allowed below for why that's fine.
...
# worker-high/worker-frontier tier config — see that file's header. Read from
# THIS script's own directory, not the caller's cwd or $REPO: a repo that vendors
# this skill carries its own copy next to the script, and during a merge-check
# run the checked-out tree is main's, so this is always main's allowlist for
# the head being evaluated, never the PR branch's (a PR could otherwise edit its
# own way past the gate).
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REVIEWER_MODELS_FILE="$SCRIPT_DIR/../reviewer-models.txt"

# True (exit 0) when free-text $1 names, as a whole token, one of the model ids
# in REVIEWER_MODELS_FILE. Case-insensitive. Tokens are runs of
# [A-Za-z0-9._-] — the character set every listed id is made of — so
# "claude-opus-5 (worker-high)" tokenizes to "claude-opus-5" and "worker-high",
# and "claude-opus-5[1m]" tokenizes to "claude-opus-5" and "1m" (brackets are
# token separators), which is why no separate `[1m]`-stripping step is needed
# here: the generator already strips it from the file, and tokenizing strips it
# from free text the same way.
reviewer_model_allowed() {
  local text="${1:-}" token
  [ -f "$REVIEWER_MODELS_FILE" ] || {
    echo "reviewer_model_allowed: $REVIEWER_MODELS_FILE not found" >&2
    return 1
  }
  while IFS= read -r token; do
    [ -z "$token" ] && continue
    if grep -qixF "$token" "$REVIEWER_MODELS_FILE"; then
      return 0
    fi
  done < <(printf '%s' "$text" | grep -oE '[A-Za-z0-9._-]+' || true)
  return 1
}
...
# one. `outcome` alone (no tab, no reviewer) means no receipt was found.
receipt_outcome() {
  local pages
  pages=$(paginate_connection comments receipt_comments_page) || return 1
  printf '%s\n' "$pages" | jq -rs --arg re "$RECEIPT_MARKER_RE" --arg reviewerRe "$RECEIPT_REVIEWER_LINE_RE" --arg head "$1" '
    [ .[] | .data.repository.pullRequest.comments.nodes[]
...
<!-- pr-review-loop:request head=$SCOPE_HEAD round=$round -->")
  ...
  merge-check)
    ...
    #
    # An approving receipt only unlocks the merge when its own reviewer field
    # names an allowed model — the tier rule in reviewer_model_allowed above.
    # A receipt written before this gate existed carries no reviewer text and
    # fails the check, which is fine: this only reads the receipt for the head
    # currently being merged, never a historical one.
    if [ "$receipt" = approve ]; then
      if ! reviewer_model_allowed "$receipt_reviewer"; then
        echo "merge=refused head=$SCOPE_HEAD verdict=$SCOPE_VERDICT: the approving substitute receipt names a disallowed reviewer (\"$receipt_reviewer\") — must be a model listed in $REVIEWER_MODELS_FILE" >&2
        exit 24
      fi
  ...
  receipt)
    ...
    # Reviewer eligibility is a tier rule, not prose: --reviewer must name one of
    # the models in REVIEWER_MODELS_FILE (worker-high/worker-frontier, whichever
    # vendor) as a whole token — never worker/worker-fast, Codex luna/terra, or a
    # flash/mini model. See docs/review-policy.md and reviewer_model_allowed above.
    if ! reviewer_model_allowed "$reviewer"; then
      echo "receipt: --reviewer (\"$reviewer\") does not name a model listed in $REVIEWER_MODELS_FILE — only worker-high/worker-frontier models may write a receipt" >&2
      exit 2
    fi
    body=$(cat "$body_file")
