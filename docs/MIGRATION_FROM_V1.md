...
Historical execution record for standing up this v2 instance alongside the
original v1 fork (`~/nanoclaw`), porting customizations, validating parity, and
cutting over.

> **Archived plan — do not execute as a current runbook.** The supported
> operator path is `bash migrate-v2.sh`, followed by `/migrate-from-v1` and
...
| **Session history loss**         | v1 sessions (JSONL transcripts, memory DBs) don't transfer to v2's schema. Conversation history is lost on cutover. | Historical mitigation was manual export. Current migration uses `/migrate-memory` for recognized durable-memory sources and never writes memory into instruction files. |
...
| **Group CLAUDE.md files**                  | `groups/*/CLAUDE.md`                            | 11 group-root files (+ global) | **Standing instructions only.** Preserve them byte-for-byte as agent-scoped customizations; do not treat them as memory or feed them to `/migrate-memory`.                           |
...
The two things that matter most remain distinct: **memories** are preserved in
the checksummed workgroup canon by `/migrate-memory`, while **CLAUDE.md files**
remain agent-scoped standing instructions. Do not merge either surface into the
other. The observations about message history and scheduled tasks below are
historical to this migration.
...
- [ ] Preserve group CLAUDE.md content only as agent-scoped standing
      instructions; reconcile it separately from memory with its own review and
      rollback.
...
> **Superseded historical experiment.** Native/provider memory is now only a
> compatibility view. The workgroup Markdown canon and automatic pre-turn
...
> **Superseded:** Do not enable a separate provider-native memory authority.
> Every provider uses the shared workgroup canon through compatibility paths.
> The text below records the old experiment only.

**Historical proposal:** v1 enabled Claude Code's native auto-memory + a shared memory directory across threads in a group.
...
- [x] Memory captures user preferences and project context — one canonical workgroup tree is shared across first wake, warm continuation, compaction, replacement, and sibling providers.
...
- [ ] Cross-thread context: a new thread can reference facts from bounded canonical or provenance-carrying archive recall admitted by the host.
