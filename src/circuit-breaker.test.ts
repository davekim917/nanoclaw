...
// `import` — so it can only use globals. uniqueTmpRoot is one: src/test-setup.ts
// installs it on globalThis, and setup files run before this module loads.
const { TEST_DIR } = vi.hoisted(() => ({ TEST_DIR: uniqueTmpRoot('cb-test') }));
