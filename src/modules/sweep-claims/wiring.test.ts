 ...
 * './modules/sweep-claims/index.js'` proves the registry mechanics but masks
...
describe('the production modules barrel registers the sweep-claims duty source', () => {
  it('the production modules barrel registers the sweep-claims duty source', async () => {
    await import('../index.js');

    const { duties } = _listSweepRegistrationsForTesting();
    expect(duties.some((d) => d.name === 'claims-reconcile')).toBe(true);
    expect(duties.some((d) => d.name === 'claims-self-heal')).toBe(true);
  });
});
