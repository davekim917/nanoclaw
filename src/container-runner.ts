...
export function buildVolumeMounts(
  ...
  if (isMain) {
    ...
    // Mount global context directory for all groups — shared instructions that every
    // agent needs. Cross-group *memory* search is separately gated by globalContext.
    {
      const globalDir = path.join(GROUPS_DIR, 'global');
      if (fs.existsSync(globalDir)) {
        mounts.push({
  ...
  const requiredSettings: Record<string, unknown> = {
    ...
    // Default to xhigh effort (Opus 4.7 deep reasoning; other models clamp
    // to the highest level they support). Overridable via /effort or
