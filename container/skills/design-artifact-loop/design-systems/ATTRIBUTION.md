...
Open Design is a local-first, open-source design tool. NanoClaw's `design-artifact-loop`
skill vendors a curated subset of its `design-systems/` corpus to use as concrete
generative targets (the "author-then-conform" mechanism). No modifications were made to
the vendored `DESIGN.md` / `tokens.css` files.
