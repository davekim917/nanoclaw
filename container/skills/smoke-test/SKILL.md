...
- **The barrier's answer.** On every `lanes` and `synthesis` fire the
  ...
  (`smoke-campaign-controller.py:2066` `publish_barrier`, whose effect writes at
  `:1479-1480` and unlinks on `doc is None` at `:1465-1473`), and every
  barrier-backed brief names that file (`:1500-1523`). `invalid[]` is artifact
  ...
  **A refusal that appears AFTER the brief was acknowledged re-offers the
  ...
  while the `.ack` is absent (`smoke-campaign-controller.py:2638-2640`), so that
  ...
  `briefedRefusal` at `:1418-1419`, carried forward at `:2631-2632`, compared by
  `_reoffer_on_new_refusal` at `:2772`) — and
  ...
  measured from the obligation's first record (`smoke-campaign-controller.py:2649`), so an owner that keeps producing
  ...
  **A refusing barrier wakes the owner on BOTH phases.** By the time a run
  ...
  without a wake (it now wakes at `smoke-campaign-controller.py:3337-3339`),
  ...
  branch declined to create (`_maybe_synthesis_overdue_blocked`, `:3381`,
  reading that obligation at `:3387-3388`), so it could not fire either.
