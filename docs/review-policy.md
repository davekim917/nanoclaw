...
On a `risk:high` or `review:requested` PR the substitute is an Opus-tier or
Fable-tier Claude model, or a different model family such as Codex/GPT, and
the receipt's `--reviewer` names the model. Sonnet-tier and Haiku-tier
reviewers give advisory reviews only, and only on other PRs. Nobody has to be
