...
- **`ncl integrations remove --delete-secret` is now subtractive-or-nothing: it refuses unless the owning group's own `container.json` is the only place the bearer is declared, and otherwise drops that declaration before deleting the vault secret.** Removal left the `onecliSecrets` declaration alone and the docs made editing it a manual step to run *first* — but the refresher re-declares the bearer on every successful refresh (`refreshOne`'s tail), so a sweep tick landing between the hand edit and the command re-declared the name the command was about to delete, and `resolveSecretUuids` then threw for that group on **every** spawn. `removeIntegrationLocked` now does the owning group's undeclare itself, inside the same per-name lock the refresher takes, so the two can no longer interleave; the order is undeclare-then-delete and the undeclare's failure aborts with nothing deleted, so re-running the command is the whole recovery. **The refusal is the load-bearing half**: a declaration also lives in `workgroups.onecli_secrets`, and the spawn grants the **union** of that and the group's own list, where neither can subtract from the other (`mergeWorkgroupAndGroupSecrets`) — so a group-level edit cannot take back a workgroup declaration or touch a sibling's file, and deleting a secret they still name would abort every spawn that inherits it, fleet-wide. `--delete-secret` therefore scans every workgroup row and every other group's `container.json` for both spellings a declaration can take (the secret's name and its vault UUID), plus any other integration in the same group pointing at the same bearer, and when it finds one it lists each site with the command to clear it and deletes nothing. Plain `remove` is unchanged: every declaration stays, and a foreign one is no reason to refuse it. Host restart only (#929).

- **The review gate no longer lets a never-started re-run hide a genuine red, and `admin=ready` answers fail-closed on three more rules.** During the Actions billing lockout a `CI (host)` success may stand in for a required workflow *GitHub never started* — but the two ways a workflow is re-run both hid an older real failure of the same check from that judgement: `Re-run jobs` keeps the run id and bumps `run_attempt` (so the failed attempt's jobs are only visible under `/attempts/<n>/jobs`), and a fresh trigger makes a second run of the same name (and `ci_verdict` keeps only the newest run per name). `never_started_runs` now requires every *attempt* of a run to be never-started and disqualifies the whole workflow name when any other completed run of it on that head is a real red — scored exactly as `ci_verdict` scores it, which is per-required-ness: `conclusion != "success"` for a required workflow (so `timed_out`, `cancelled`, `neutral`, `skipped` and `startup_failure` all count), and not-in-success/neutral/skipped for every other one, so an ordinary `concurrency: cancel-in-progress` on a non-required workflow does not cost it its excuse. None of them costs a jobs read. `required_status_red` no longer excuses a check run whose own job never started unless that run is one `never_started_runs` vouches for. Anything unreadable disqualifies rather than being skipped. Separately, `admin_readiness` gains three fail-closed answers at no extra API cost: a non-author `CHANGES_REQUESTED` review, `strict_required_status_checks_policy` (GitHub's "require branches to be up to date", which it does not evaluate), and GitHub's own `reviewDecision` being anything but `APPROVED` or null — which replaces the approval *arithmetic* that used to live here, because `reviewDecision` already counts only write-access approvals. It does **not** replace reading the base rules: the rules' own `required_approving_review_count > 0`, and the extra approval `require_extra_approval_for_unattributed_changes` owes, each still refuse on their own. Measured, not assumed — a production repository's ruleset is active on its default branch with that second parameter on, and an open PR against that branch with three unattributed commits reports `reviewDecision: null`, so `reviewDecision` does not surface that rule's review parameters and the sibling parameter in the same rule object cannot be assumed to. Script-only; no deploy (#937, partial).

- **The staged OpenCode default-model config now actually reaches agent shells.** #886 wrote `opencode.json` under `$XDG_CONFIG_HOME/opencode/`, but `container/entrypoint.sh` re-exports `XDG_CONFIG_HOME=/tmp/.chromium` (a crashpad workaround) before exec'ing the runner, so every agent shell looked for the global config there and a bare `opencode run` still fell to OpenCode's own guess (#887, shadow review of #886; the #886 proof ran from a `docker exec` shell, which skips the entrypoint). Every container's env now also carries `OPENCODE_CONFIG=/opencode-xdg/opencode/opencode.json`, which names the file explicitly and survives the entrypoint. Host restart only.

- **Codex subagents in containers no longer carry a global `default_subagent_reasoning_effort`.** The generated container `config.toml` (`src/providers/codex.ts` for the staged home, `container/agent-runner/src/codex-companion-setup.ts` for the runner-written and fallback homes) pinned every Codex subagent to `high` unless its spawn or role said otherwise. The operator removed that key from the host's own `~/.codex/config.toml` on 2026-09-17; the container config now matches, so a subagent runs at Codex's native default unless `spawn_agent` names a `reasoning_effort` or its role carries `model_reasoning_effort` (the `/orchestrate` shims do). Host restart only.

- **A bare `opencode run` in any container now runs the fleet's OpenCode default model.** The staged OpenCode tree every container carries (`/opencode-xdg/opencode/`, #874) held only `auth.json`, so a headless `opencode` with no `-m` had no configured model and no last-used one and fell to OpenCode's "first model by internal priority" across every credentialed provider — in a Codex-group container, which also carries `OPENAI_API_KEY`, that picked an OpenAI model whose OpenCode OAuth entry on the host was stale and failed with `Token refresh failed: 401` (#884). `stageOpenCodeAuth` now also writes `opencode.json` with `model: opencode-go/deepseek-v4.1-flash` (`DEFAULT_OPENCODE_MODEL`). `-m <provider/model>` still picks anything else; the OpenCode provider's own container is unaffected because its full config arrives as `OPENCODE_CONFIG_CONTENT`, which OpenCode merges over the global file. Host restart only.

- **A spent Codex secondary no longer parks the whole provider until the secondary's reset, and a `-m` pin from the primary provider no longer follows the session onto its fallback.** The Codex OAuth ring (`~/.codex` plus `codexAuthFallbacks`) walked forward only for the life of a container: once a query had rotated to the last fallback, that account's next park ended the turn with nothing left to try, and the host parked `(agent_group, 'codex')` until *that* account's weekly reset while the primary had recovered turns earlier — a Codex group sat on its Claude fallback with a healthy primary on 2026-09-16. The ring is now circular per turn (`CodexProvider.rotateCodexHome`): every account is retried once per turn, only a turn on which all of them fail reaches the host, and that report carries the ring's *earliest* stated reset (`earliestCodexSlotReset`). Separately, the runner now ignores a stored model pin that belongs to another provider (`modelBelongsToProvider`, `applyFlagBatch`): a sticky `gpt-6-astra` from `-m astra` earlier in a thread had every turn of the Claude fallback ask the Anthropic API for it. The pin stays in session_state and applies again when the primary is back; one `⚙️` line in chat says it is being ignored. Deploy is a host restart, no image rebuild — the agent-runner source is a boot snapshot (`src/agent-runner-source.ts`).

- **New: `ncl integrations` — connect a remote MCP server over standard MCP authorization instead of pasting an API key.** `login` probes the MCP URL, reads the `WWW-Authenticate` challenge, walks the protected-resource and authorization-server metadata, registers a client dynamically (RFC 7591), and runs an authorization-code flow with PKCE S256; `complete` exchanges the code the operator pastes back. The access token lands in the same OneCLI header-injection secret the container's bridge already reads, and the secret name is appended to the group's `container.json` `onecliSecrets` so the spawn path actually grants it — the half that is easy to miss, since a secret that exists in the vault but is not declared in `groups/<folder>/container.json` is never granted to that group's OneCLI agent. A new sweep duty (`mcp-oauth-refresh`, `tick:housekeeping` order 27) re-mints the access token inside a 10-minute expiry margin and PATCHes it over the same secret, so a running container picks it up on its next request with no restart; refresh-token rotation is persisted before the row moves, and an `invalid_grant` marks the integration `needs_login`, warns once, and stops retrying. Every OAuth endpoint must be `https` (RFC 8414 §2), discovered or supplied by `--issuer`/`--device-endpoint` — metadata arrives over the network, so an `http:` token endpoint in it would put the authorization code and refresh token in cleartext. Registry table via migration 082 (`mcp_oauth_integrations`) carries endpoints, scopes, secret names, expiry and status — **no token material**. **The refresh token and client secret live host-side** — and only those; the access token is never written outside OneCLI — at `data/mcp-oauth/<name>.json` mode 0600 in a 0700 directory, not in OneCLI: `onecli@1.4.1`'s gateway API is write-only for secret values (`PATCH /api/secrets/{id}` works; there is no read route), so a refresh token parked there could never be read back — the same shape as the GitHub App private key the host already holds on disk to mint installation tokens from. **Deploy is host-only: restart, no image rebuild.** `--listen` (plus an `ssh -L` tunnel) and `--device` are opt-in alternatives to the default paste flow. See [docs/mcp-oauth-integrations.md](docs/mcp-oauth-integrations.md).

- [BREAKING] **The Slack user-token MCP (`korotovsky/slack-mcp-server`) is removed; live Slack is `curl https://slack.com/api/<method>` through the OneCLI proxy, and the Slack capability entry can no longer be budget-dropped.** The MCP failed to connect on every spawn (`failedMcpServers: slack-user-token` since at least 2026-08-31) and no agent ever called it, while every real Slack read and write went through curl with the gateway-injected token. The harm was its failure notice: on 2026-09-16 an agent saw it, found no Slack entry in its capabilities because the pre-turn capability budget had dropped it (`capability-total-budget: … Slack (read)`), and told the owner Slack was unavailable. The credential boundary is unchanged — a non-owner-safe session still spawns under the `<group>-noslack` OneCLI identity with the token withheld. `slack_user_token.enabled` is retired (accepted, ignored); `also_allowed_in` and `onecli_secret_names` keep their meaning. The Slack capability text now gives the permalink-to-`ts` recipe and the methods, and carries `retainUnderBudget`, which the host's capability budgets and the runner's fresh-context fallback evict last. **Deploy needs an agent image rebuild and a host restart.** **Migration:** [docs/slack-user-token.md](docs/slack-user-token.md#migrating-off-the-slack-mcp) (detect / why / fix / verify / rollback).

- [BREAKING] **The `opus` alias now means Opus, and the fleet defaults move: Claude to Opus 5 [1m] at `high`, Codex to `gpt-5.6-sol` at `high`.** `ANTHROPIC_DEFAULT_OPUS_MODEL` — the SDK short-circuit deciding what the bare word `opus` resolves to — carried each group's own resolved model rather than an Opus id, so "opus" meant "whatever this group runs". When the unpinned Claude default moved to Sonnet 5 on 2026-09-14, the `"model": "opus"` pin that group-init writes into **every** group's `.claude-shared/settings.json` started resolving to `claude-sonnet-5` (measured live in a running container), and so did every `model: opus` subagent. The three `ANTHROPIC_DEFAULT_<FAMILY>_MODEL` vars are now install-wide constants in every group, and the group's own model travels as `NANOCLAW_CLAUDE_MODEL`. Alongside it, the unpinned defaults for both providers: a Claude group with no model configured resolves to `claude-opus-5[1m]` (was `claude-sonnet-5`) and keeps Opus's `high` family effort, and a Codex group with no model configured resolves to `gpt-5.6-sol` at `high` reasoning (was `gpt-5.6-terra` at `xhigh`) — one pair of constants, `DEFAULT_CODEX_MODEL`/`DEFAULT_CODEX_EFFORT` in `container/agent-runner/src/providers/codex.ts`. **This changes production behaviour on deploy** — every unpinned Claude group's conversations move from Sonnet 5/`xhigh` to Opus 5 [1m]/`high` at Opus pricing, and a Codex group moves on whichever half it never configured: a group with no model moves Terra → Sol, and one with no `reasoning_effort` moves `xhigh` → `high` (on this install every Codex group already sets `effort`, so only the model moves). A declared `providerFallback` moves too — it resolves through these same defaults — which on this install is 14 fallback paths that a primary-provider check would not list. Groups, channels and fallbacks with an explicit pin keep it, in both providers; no pin is ever rewritten. OpenCode defaults are untouched. **Migration — pin BEFORE you deploy:** [docs/claude-default-model.md](docs/claude-default-model.md) (detect / why / fix / verify / rollback, both providers); pins are data and need no deploy, the defaults are code and need one.

- **Background agents no longer die with an idle-reaped task container.** A Claude turn that launched a subagent with `run_in_background` and then ended — typically on a `wait` — published `provider_executing = 0` while that agent was still running inside the CLI, and the host's scheduled-task idle reaper killed the container within one sweep tick; the next wake found "Background agent … didn't finish before the previous session ended" and the agent re-delegated, forever (2026-09-15: one task session reaped nine times in 80 minutes, every worker lost). The runner now mirrors the CLI's `background_tasks_changed` level set (`AgentQuery.hasBackgroundWork`), holds the busy level at `result` while any non-ambient background task is live, and lowers it on the CLI's idle signal — which that CLI withholds until background agents are done and any follow-up turn they start has run — so there is no gap in which a sweep could reap the container before the completion is processed. The CLI gates its idle on background subagents only; a backgrounded Bash command or a monitor is held until it completes and no longer, as before. A model/effort change admitted between turns also defers its stream restart while background work is live. Needs the pinned CLI (`2.1.272` emits the message; an older CLI never populates the set and behaves as before). Takes effect at the next host restart (runner source is a boot snapshot).

- **OpenCode fleet default model moved to `opencode-go/deepseek-v4.1-flash`** (was `opencode-go/glm-5.3-flash`; effort stays `high`). `DEFAULT_OPENCODE_MODEL` in `src/providers/opencode.ts` is the only place the default lives — the `provider_models.is_default` row was dropped with migration 039 — so every OpenCode group with `model: null` moves on its next spawn after deploy; a group with an explicit `--model` pin is untouched. **Prerequisite before deploy:** DeepSeek models on Go are China-hosted and the OpenCode workspace must opt in (the Go endpoint refuses `opencode-go/deepseek-v4.1-flash` and `opencode-go/deepseek-v4-flash` with "requires explicit opt in" until then, even though `opencode models` lists both). Rollback is pinning `--model opencode-go/glm-5.3-flash` per group, or reverting the constant.

- **New: a checkout per thread and branch behind `NANOCLAW_CHECKOUT_MODE=clone`.** In `clone` mode, `create_worktree` with a `branch` returns the thread's checkout at `/workspace/worktrees/<repo>` when it is already on that branch, otherwise that branch's own independent clone at `/workspace/worktrees/<repo>@<branch-slug>`, so threads no longer block each other on one shared checkout. The host builds each clone, hardlinking the canonical's objects and, with `NANOCLAW_DEPENDENCY_CACHE=apply`, linking verified shared `node_modules` farms; it needs containers that run as the host uid. The default mode, `worktree`, is unchanged: one linked worktree per thread and repository, and a request for another branch is refused as before. In both modes `git_commit`, `git_push` and `open_pr` take `branch` to select the checkout they act on, defaulting to the primary. Rollback is flipping the flag back and restarting: clones stay usable in `worktree` mode. Worktree cleanup collects an idle clone only after proving that the commits of every local ref, HEAD and the stash are already on `origin`; a tag the clone still holds exactly as the host recorded it when building the clone is not counted as its own work. No migration: existing linked worktrees keep working in both modes. See [docs/workgroups.md](docs/workgroups.md#canonical-repositories-and-topic-worktrees).
