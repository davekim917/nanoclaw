...
- **`src/group-init.ts`** — the agent-runner source is a boot snapshot (`src/agent-runner-source.ts`) mounted read-only for every group; edits reach a group at its first spawn after the next host restart — an adopted container keeps its old snapshot until it exits (Container Restart).
...
**`on_wake` closes a race**: only picked up on a fresh container's *first* poll, so a dying container still in its SIGTERM grace period can't steal it — `killContainer`'s `onExit` guarantees the old process is gone first. **Ceiling kills are accounted for publicly**: the 30-min idle ceiling (`ABSOLUTE_CEILING_MS`) killing resumable work queues a deferred `ceiling-respawn-*` wake; the fresh container must post done/lost/next (narration is never recovery evidence), capped at `WORK_CONTINUATION_RESUME_MAX_ATTEMPTS` (2) — `decideCeilingFollowUp` in `src/host-sweep.ts`. `continue_work({ task })` (`session_state.work_continuation`, chain cap 50) is the only sanctioned way to promise follow-up — future-tense prose has no control effect; full semantics: [docs/agent-runner-details.md](docs/agent-runner-details.md). `wait` wakes in-thread, same session; `ncl tasks` runs in its own task session, and each scheduled fire starts a fresh conversation unless the series is `--continuous` (`container/agent-runner/src/fresh-context-task.ts`). **A host restart does not stop running containers**: `planContainerShutdown()` leaves every one for the next host to adopt (`adoptRunningSessions`, `src/container-runner.ts`; the systemd drop-in sets `KillMode=mixed` and an empty `ExecStop`). Boot stops only a container it cannot adopt — over the memory admission budget, a duplicate for its session, or a session that cannot take a wake — and only those interrupted sessions get a deduped `host-restart-*` note (`src/host-restart-warn.ts`). An adopted container keeps its spawn-time image, env and runner snapshot until it exits, so a deploy reaches a running session only at its next spawn.
...
**CI is not your test run.** A PR only gets typechecks plus the review-notes and upstream-ratchet tests (`.github/workflows/ci.yml`); the full suite runs nightly (`.github/workflows/ci-full.yml` — ~20 min on GitHub's 2-vCPU runner against a 2,000 min/month quota, so it cannot run per push). Correctness before merge is the author's: run `flock <scratchpad>/vitest.lock ionice -c3 nice -n 10 pnpm run lint` (typed eslint; CI no longer lints a PR) and the test files for what you touched, throttled, never the full suite on this host — `flock <scratchpad>/vitest.lock ionice -c3 nice -n 10 node_modules/.bin/vitest run <files> --maxWorkers=2` — and say in the PR which files you ran. A red nightly opens or updates the issue "Nightly CI is red on main"; fix forward the same day. `gh workflow run ci-full.yml --ref <branch>` runs the full suite on demand for a change that warrants it, and the merge gate honours that run: red or still running blocks the merge. When Actions cannot start jobs at all (billing lockout), `container/skills/pr-review-loop/scripts/run-host-ci.sh --pr <n>` runs `.github/host-ci.sh` (ci.yml's PR lane, drift-tested) on the exact head and posts `CI (host)`, which merge-check accepts only for a workflow GitHub never started (`ci=host`) and only from an allowed poster (`CODEX_REVIEW_HOST_CI_POSTERS`, default the gate's own account).
...
- **Display**: anything shown to an agent or user renders in the install timezone via `formatLocalTime`/`formatLocalStamp` (`src/timezone.ts`, runner `timezone.ts`); `--json`, DB values and setup logs (`logs/setup.log`, `logs/setup-steps/`) stay ISO. Host log lines (`logs/nanoclaw*.log`) are stamped in the host process's local time — its `TZ` env, else `/etc/localtime` — followed by an explicit numeric UTC offset (`ts()`, `src/log.ts`): `[2026-09-22 08:50:37.011-04:00]`. That zone routinely differs from both the install timezone and the zone of whatever reads the line (this install's unit sets `TZ=America/New_York` while `/etc/localtime` is `Etc/UTC`), so **never rebuild the instant from the wall-clock parts** — parse with `parseLogStamp` (`src/log.ts`), which returns `exact: false` for lines predating the offset. Rotated logs keep 30 days, so offset-less lines stay in circulation until 2026-10-22; for those there is no recoverable instant, and a consumer must say so rather than print a confident count. When grepping by hand, convert your UTC window first, or the grep matches nothing and reads as clean.
