...
- [BREAKING] **Scheduled tasks no longer have their own model/effort default.** An unpinned scheduled-task fire on a Claude group used to be forced onto `sonnet` at `xhigh` effort, regardless of what the group was configured to run; it now resolves to the group's configured model and effort, else the provider's per-family default — the same values interactive chat starts from. It deliberately does NOT pick up a per-session sticky `-m`/`-e` a human set in chat: a scheduled fire is not that conversation, and a nightly task inheriting yesterday's `-m opus` would be the same invisible-default problem in a new place. A task's OWN `--model`/`--effort` pin is unaffected and still wins. Codex and OpenCode were never in this branch. **Who is affected:** any Claude group with _unpinned_ scheduled tasks; those series move to their group's default, which is a behavior AND per-fire cost change (commonly to Opus). **Most installs need no action** — every affected task keeps running. Nothing is auto-pinned on upgrade, on purpose: a pin that changes by itself is not a pin. **Migration:** [docs/scheduled-task-defaults.md](docs/scheduled-task-defaults.md) — how to list what moves (`ncl tasks list` now has a `PIN` column), how to pin a series deliberately if you want the old behavior, how to verify at the per-turn ledger rather than at the config store, and the rollback caveat that a pin cannot currently be cleared from the CLI.
