...
- **The barrier's answer.** On every `lanes` and `synthesis` fire the
  ...
  (`smoke-campaign-controller.py:2117` `publish_barrier`, whose effect writes at
  `:1530-1531` and unlinks on `doc is None` at `:1516-1524`), and every
  barrier-backed brief names that file (`:1551-1574`). `invalid[]` is artifact
  ...
  **A refusal that appears AFTER the brief was acknowledged re-offers the
  ...
  while the `.ack` is absent (`smoke-campaign-controller.py:2689-2691`), so that
  ...
  `briefedRefusal` at `:1469-1470`, carried forward at `:2682-2683`, compared by
  `_reoffer_on_new_refusal` at `:2823`) — and
  ...
  measured from the obligation's first record (`smoke-campaign-controller.py:2700`), so an owner that keeps producing
  ...
  **A refusing barrier wakes the owner on BOTH phases.** By the time a run
  ...
  without a wake (it now wakes at `smoke-campaign-controller.py:3388-3390`),
  ...
  branch declined to create (`_maybe_synthesis_overdue_blocked`, `:3432`,
  reading that obligation at `:3438-3439`), so it could not fire either.
