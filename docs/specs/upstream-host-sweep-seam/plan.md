...
- **L-2** `src/host-lifecycle.test.ts` (fork-owned copy of upstream's five `host module lifecycle registry` cases, verbatim — §4.1) › "start callbacks run FIFO and a throw propagates; shutdown callbacks run LIFO and a throw is swallowed" — upstream's assertions, unmodified; the three fork-inapplicable upstream cases are replaced by L-3 (two) and deferred (approvals, one).
