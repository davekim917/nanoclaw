...
- `add_mcp_server({ name, command, args?, env? })` or
  ...
  Credential headers must be exactly `"onecli-managed"` or an auth scheme
  followed by it (`"Bearer onecli-managed"`); the OneCLI gateway substitutes
  the real secret at the proxy boundary. A remote URL is stored verbatim, so
