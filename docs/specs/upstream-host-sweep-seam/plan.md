...
What the operator sees: nothing. No cadence change, and no schema change through PR 14. S2-PR15 adds exactly one: migration 068, a nullable `sessions.sweep_quiet_until` with no backfill, written and read only by the sweep's own quiet cache. What changes for the program: the 2,438-line both-touched core file that is the fork's largest remaining merge-conflict surface becomes under 300 lines of driver plus module registrations, and the ledger's `onHostStart` row (215 features / 37,074 lines across G26, G28, G30, G32, G45, G48, G57, G13, G39, G23) finally gets the seam it was booked against.
...
- **`resolveSession`, the `sessions` partial UNIQUE indexes.** No change to session resolution or lookup semantics, so critic finding B2 stays dormant exactly as it did for seam 1. Central-DB migrations: zero through PR 14; S2-PR15 adds one additive nullable column (068) and touches no index and no lookup path.
...
Three surfaces, one per shape the source actually has. The two hook lists exist because a duty can run inside a window **another duty owns**: the OOM notice runs inside the SLA's observe read, and the follow-ups run inside the session the SLA opens after the kill. Neither can be a phase — the kill respawns through `onExit` and clears status through `delivery.ts`, both of which open a session on the same key, so holding one across the kill throws (constraint 18). **(B3 final r2)** "After the kill" means after the container's exit, not after the kill request: the post-kill session opens off `killContainer`'s `onExit` callback, not off the call that issues the kill — a pre-existing gap in the driver, surfaced by the ownership early-out above.
...
**#359 (B3 final): S9b's wake dispatch is detached.** The per-session due-message wake — S9b, owned by `sweep-continuation` (S2-PR13, §5) — starts `wakeContainer` DETACHED inside W2; the sweep loop never awaits a spawn. `reportWoke(true)` gates observe (W3) and the quiet-mark decision for that tick. The work-continuation restore hangs off the wake promise rather than the tick; a deferred restore refused by the outbound-ownership guard — a container took the session during the 20–47 s wake — is logged at info and the continuation attempt STANDS, because the now-running container owns and works the continuation record itself. The tick-timing line carries three new fields — `wakesStarted`, `spawnsAwaited`, `spawnWaitMs` — so a slow spawn is visible separately from slow sweep work (§6, §10). S9b keeps at most one detached wake follow-up outstanding per session across ticks — a session already carrying an unresolved detached wake is not handed a second one on the next tick.
...
- **`quietSessions`** (`:121`) stays in `host-sweep.ts` as fan-out machinery: bounded at `sessions.length + 500`, invalidated by `session.last_active`, hint computed after the last phase. It has a time bound and `last_active` invalidation but deliberately **no mtime change-signal** — a re-home keeping only the mtime half loses the invalidation. **Amended by S2-PR15 (#320, 2026-09-03):** the time bound is jittered per session (deterministic, hash of the id) so the herd does not expire on one 30-minute grid, and the mark is persisted in `sessions.sweep_quiet_until` and warmed at `startHostSweep`, so the first tick after a boot does not scan every active session; the `last_active` invalidation still wins over a persisted mark. No new duty (marks live on session rows; nothing to prune). **As built:** the jitter band is [0.5, 1) x the bound, so it only ever SHORTENS a skip and the 30-minute ceiling above still holds for every session; the persisted column is migration 068 and the `last_active` invalidation wins because `updateSession` nulls it in the same statement that writes `last_active`, which is the durable half of the in-memory `mark.lastActive === session.last_active` check; the never-past-`getNextFutureProcessAfter` bound and the `sessions.length + 500` bound are untouched; and `skipUnreadable`'s backoff is deliberately NOT persisted, because its causes are process-local and a restart is the event that clears them. **The synchronous invalidation mechanism (B3 rounds 2–4):** every due-ness writer invalidates the persisted mark SYNCHRONOUSLY inside its own open mailbox callback, immediately before the SQLite mutation that puts due work back, through `withQuietInvalidationSync` — one UPDATE that both advances `last_active` (strictly) and sets `sweep_quiet_until = NULL`, fail-closed unless exactly one active, non-archived row changed. There is no post-write touch: the invalidation runs before the mutation it guards, and the flush's `IS <basis>` guard rejects any in-flight mark computed on a `last_active` value the invalidation has since moved past. Writers: `scheduleTask` (primary and retry paths), `createScheduledTask`, the five `scheduled-mutations.ts` dashboard mutation handlers, both scheduled-move restores, `ncl tasks` mutations (probe-then-bracket), and both recurrence re-arms. **Addendum:** `sessions.last_active` is monotonic per session — `updateSession` never writes a value smaller than the one already stored, and the invalidation helper always writes strictly greater — which is the property the flush's `IS <basis>` guard depends on to reject a stale mark.
...
| `:1840` | `rollupSessionUsage` (`db/usage.ts`) | PR 6 (signature); the sweep's raw outbound open was a deliberate KEEP-PATCH until PR 7 (#297, `5f6a99ab`) added the outbound-keyed read-only funnel `readSessionOutbound` — S2-PR12 converts T19 to it at that merge-down and leaves the ratchet (see F-12.2) | S2-PR12 |
...
**~~One permanent KEEP-PATCH~~ — retired at the B3 merge-down onto mailbox PR 7 (2026-09-03).** The premise was that scheduled-move recovery walks an INJECTED sessions root; PR 7's cited facts show no production caller ever injects one (the sole site is `recoverMoveIntents(getDb(), {})`), and on PR 7's head the injected root already split reads from the restore (which resolves via `DATA_DIR`). Seam 2 therefore drops `MoveRecoveryOptions.dataDir`, makes `DATA_DIR` the single root, converts the restore as PR 7 did, and `sweep-scheduled-move/index.ts` leaves the ratchet — the host half of the allowlist is exactly `['src/storage-manager.ts']` on both seams. Original text kept for the record: `host-sweep.ts:54` imported `openInboundDb as openInboundDbByPath` from `src/modules/mailbox/openers.js` for the scheduled-move recovery, on the claim that it could never go through the seam: *"The scheduled-move recovery below walks an INJECTED sessions root, not `DATA_DIR`, so its session DBs are not addressable by a mailbox key."* S2-PR7 carries the import into `src/modules/sweep-scheduled-move/` with that reason restated at the import site, and keeps the file on the ratchet allowlist. It is a deliberate exception with a written cause, not residue to eliminate — the same class as `storage-manager.ts`'s worker-thread reclaim probe, which mailbox PR 5 documented as the sanctioned PR 7 exception. Acceptance case F-7.3 asserts the behavior the exception exists for.
...
| S2-PR15 quiet cache (#320) | S2-PR14 | S2-PR14 | driver machinery only; needs PR 14's final driver shape. Moves no duty |
...
| S2-PR15 quiet cache (#320) | — (driver machinery) | ~144 | 20 new | worker-high | 0.5 |
| | **38** | | | | **12.4** |
...
**S2-PR15 is the one exception to the quiet-cache ban, because it *is* the quiet-cache PR.** It is not a family PR: it moves no duty and registers none. It owns the cache end to end — `src/host-sweep.ts`'s cache machinery, `src/db/sessions.ts`'s mark helpers and the `last_active` clear inside `updateSession`, migration 068, `Session.sweep_quiet_until`, and the quiet-cache cases in `src/host-sweep-registry.test.ts` (including F-14.1's export allowlist and regrowth ratchet, amended in §8).
...
- **Rollback**: `git revert <merge>` → build → restart, under 10 minutes. Through PR 14 there are **no migrations**, so there is no data to restore and both code versions read the same files. S2-PR15's migration 068 does not change that: the column is additive and nullable, the reverted code never reads or writes it, and the rows it leaves behind are inert. Rollback stays code-only — the column is **not** dropped.
...
grep    'Host sweep tick timing'        logs/nanoclaw.log | tail  # tick duration within the pre-PR envelope; #359 boot gate below
```

**#359 boot gate** (S2-PR13's detached wake, above): once `wakesStarted` field is present in the tick-timing line, `(sessionsMs − spawnWaitMs) / sweptSessions` must stay ≤ 55 ms at load < 4 — a rising figure means a detached spawn is being awaited somewhere it should not be.
...
- **T-1** `src/host-lifecycle-timers.test.ts` (renamed from `host-sweep-registry.test.ts` at B1, 2026-09-03 — S2-PR2 owns that name) › "main.ts starts no duty timer directly" — grep `src/main.ts` for the seven start/stop symbols; assert zero matches.
...
- **T-4** `src/host-lifecycle-timers.test.ts` › "module intervals are unref'd and cleared on shutdown" — after `startHostModules` then `stopHostModules`, no timer keeps the loop alive and each interval had `unref` called.
- **T-5** `src/main.test.ts` › "after PR 1 the registries hold exactly the six timer starts and the host's shutdown registrants, regardless of env gates" — (corrected 2026-09-03 at B1: the original counted 6/7 and went red on main when #324's archive-projection worker registered an eighth shutdown — a count is a shape test that breaks on every legitimate registrant and names nothing) with a fresh module registry, import the modules barrel and the six timer modules; every registration site registers a **named function expression** (`host-lifecycle.ts` stays byte-identical to upstream), and the case asserts the exact `fn.name` sets: six starts, and the shutdowns = six timers + storage maintenance + archive-projection worker; repeat with `DAILY_SUMMARY_ENABLED=0` and `BACKLOG_CANVAS_ENABLED=0` and assert the same sets (a disabled duty registers and no-ops, §4.2). A new registrant is added by name. Replaces L-4 from PR 1 onward.
...
- **R-7** same file › "the registered duty set matches the seam-2 inventory" — pinned at 41 registrations across 40 distinct names as of `origin/main` (the 38-duty seam-2 port registers 39 times, S17 twice, plus #285's `cli-request-execution-prune` and #247's `github-token-file-refresh`); the name set equals the checked-in inventory id set.
...
- **R-9b** same file › "a housekeeping duty that touches a session invalidates the mark that tick took" — (reworded at the B3 round-2 fix, `667a6f12`: the case originally re-swept in-process before restarting, which re-took a fresh, legitimate mark and made the restart assertion pass for the wrong reason) asserts only the across-restart arm: a mark this tick took and a same-tick `tick:housekeeping` duty then cleared (standing in for `recoverMoveIntents`' restore and scheduled-move's compensation) must not survive a restart with no intervening sweep. The in-process arm — a `last_active` change invalidating the mark immediately, in the same tick — is R-9's own assertion and is deliberately not re-driven here.
...
- **F-7.3** same file › "move recovery reads and restores through one sessions root" — (rewritten at the B3 merge-down; the original asserted an injected-root contract that no production caller uses and that PR 7's head silently splits) the live-count read, the inbound.db existence pre-check and the restore all resolve the SAME sessions path for a session — the property the split-root shape violates; `MoveRecoveryOptions.dataDir` no longer exists; the module is off the ratchet allowlist.
...
- **F-11.2** same file › "recurrence runs before the spent task-session GC" — a just-fired recurring series is not collected in the same tick. **(B3 final r2)** S19 never closes a session referenced by an unresolved move intent — a fail-closed read of `scheduled_audit` via `hasUnresolvedMoveIntent` — so T11's crash recovery can still restore that session through the fail-closed invalidation on the next tick.
...
- **F-11.4** same file › "thread-close advance stops the container, then clears saved work and archives, in that order" — (retitled at the B3 merge-down onto mailbox PR 4 round 8; formerly "thread-close advance clears saved work, stops the container and archives, in that order") the expected call order under the guarded finalizer is `['kill', 'clear', 'archive']`, mirroring upstream `src/dashboard/thread-close.test.ts:510` › "kills first, then clears once the process is gone, then archives"; the old `['clear', 'kill', 'clear', 'archive']` order was unreachable once ownership is read through `containerOwnsOutbound`.
...
- **F-12.2** same file › "the usage mtime cache is written only after a rollup actually ran" — (corrected at build against mailbox PR 6's final head `a49b8172`, where the rollup deliberately keeps the module's raw outbound opener because the seam's existence check is keyed on inbound.db) inbound.db gone while outbound.db remains → the rollup STILL runs on outbound alone and the cache updates (constraint 21's purpose); a rollup that fails before completing → cache unchanged, preserved log, re-attempted next tick; a real rollup updates the cache and an unchanged mtime skips on the next tick. The sweep-usage module carried the raw opener behind a ratchet entry until the merge-down onto mailbox PR 7 (#297, `5f6a99ab`), whose outbound-keyed read-only funnel `readSessionOutbound` (`src/modules/mailbox/read-only.ts`) is the op the KEEP-PATCH was waiting for; T19 now opens through it (same shape as host-sweep.ts's own rollup on PR 7's head), the three-part case is unchanged in title and assertions, and `sweep-usage/index.ts` left the allowlist. scheduled-move (F-7.3) remains the only permanent KEEP-PATCH.
...
**S2-PR14 — final (2)**
- **F-14.1** `src/host-sweep-registry.test.ts` › "host-sweep.ts contains no inline duty bodies" — structural, corrected at build (2026-09-03, the "under 300 lines" figure was a guess: the driver + registry + phase list + shared context + error rule + quiet cache are 1,137 lines / 631 code at the integrated head, with no duty body): importing `src/host-sweep.ts` alone yields zero registrations; the file has no `registerSweepDuty(`/`registerSweepDutySource(` call sites outside the registry API save the driver's own single empty built-in source (`registerSweepDutySource('host-sweep:builtin', registerBuiltInSweepDuties)`, registering nothing — the one permitted call site); its export set is a subset of the declared driver/registry/phase-list/context/error-rule surface; and a regrowth ratchet with the section breakdown in a comment. No driver split. **Amended by S2-PR15 (#320):** the export allowlist gains `QUIET_SESSION_BACKOFF_MS` and the two test accessors `_resetQuietSessionCacheForTesting` and `_lastSweepTickStatsForTesting`. **Ratchet ceiling corrected at the B3 fixer round to 1,400** (measured 1,347 at B3 — up from the 1,200 → 1,300 estimated at S2-PR15's own build — a +149 delta over `dc440893`'s 1,198, with the breakdown recorded in-file: ~90 of the 149 lines are S2-PR15's jitter and boot-warm machinery, and the guard relocation elsewhere in the file is a wash). **Raised again at B3 final (06:45Z) to 1,450** — the driver measured 1,410 after the #359 tick-timing instrumentation landed (S2-PR13, below), a +55 increment over the 1,347/1,400 prior measurement spent on the `wakesStarted`/`spawnsAwaited`/`spawnWaitMs` fields the tick-timing line now carries. The four structural assertions are unchanged — the ratchet is a regrowth guard against a duty body coming home, never a line budget.
...
- **Q-3** › "a warmed mark never outlives its next due row" — the persisted value IS the due row when the due row is sooner than the cap; warm past it and assert the session is swept.
- **Q-4** › "a session with a live container is never warmed as quiet".
- **Q-5** › "the quiet mark is written on the transition only, not on every confirming tick" — three consecutive quiet ticks, exactly one persistence write.
- **Q-6** › "a failed persistence write degrades to a cold sweep and never to a skipped due session" — the write throws; assert the warn line, no throw out of the tick, and that the in-memory marks went with it so the next tick sweeps.
- **Q-7** › "quiet backoff is jittered so one cohort does not expire in one tick" — 200 sessions marked in one tick; observed expiry spread exceeds 10 minutes and no value exceeds `now + QUIET_SESSION_BACKOFF_MS`.
- **Q-8** › "quiet marks expire on a jittered schedule, never all on one tick" — spread ≥ 20 % of the backoff window, no single tick takes the whole cohort, and the spread reproduces exactly on a second run (the jitter is a hash, not `Math.random`).
...
- **Q-19** `src/db/scheduled-tasks.test.ts` › "clears sweep_quiet_until and advances last_active on the task session" — `scheduleTask`'s own invalidation, which had no coverage because the suite's fixture lacked the column and the invalidation call (then `touchSessionActivity`, since replaced by the fail-closed `withQuietInvalidationSync`, §4.4) swallowed its own errors (Codex round 2, L1). The fixture now carries `sweep_quiet_until`, in that suite and in `scheduled-move`'s, with a comment saying why it is load-bearing rather than scenery.

**S2-PR16 — archive-projection merge-down (#324) (3)**

No PR16 code. Scoping (`reports/s2-pr16-scope.md`) found the storage walk already runs in a `worker_threads` Worker and the long ticks were serialized container spawns whose synchronous projection build `main` had already moved off-thread (#324); the seam-2 lineage inherits that through its base. What is owed is verification that the integration did not lose it, so the three cases are carried in batch B3 against the integrated tree.

- **F-16.1** `src/db/archive-projection-worker.test.ts` › "a fresh projection is reused and a stale one is rebuilt on a worker thread" — #315's decision, both halves in one case (either alone passes for the wrong reason), and every build goes through the worker seam rather than the in-process fallback.
- **F-16.2** `src/modules/sweep-storage/storage.test.ts` › "the host thread never calls buildArchiveProjection directly" — source-text over `src/container-runner.ts`: zero `buildArchiveProjection(` call sites and exactly one awaited `ensureArchiveProjection(`. The hazard is a merge, not a feature: a conflict resolution that reinstates the synchronous call puts a multi-second blocking build back in front of every spawn.
- **F-16.3** `src/host-lifecycle-timers.test.ts` › "the projection worker and the storage worker each stop exactly once" — after `startHostModules` then `stopHostModules`, each worker's `terminate` is called exactly once and no storage-teardown failure is logged. Re-proves Codex's PR 6 F1 shutdown-ownership finding across the second persistent worker.

**Deploy gate for #324's effect** (read after the next host restart, not from a tick minutes old):

```bash
grep 'Spawn stage timing stage="archive-projection"' logs/nanoclaw.log | tail -40   # median well under 2 s, was ~22.6 s
grep 'Host sweep tick timing' logs/nanoclaw.log | tail -60                          # sweepMs < 30000 whenever sweptSessions < 20
grep -E 'Waking container for due messages|Spawning container sessionId' logs/nanoclaw.log | tail -40   # wake→spawn median falls from 19.6 s
grep 'Sweep delivery poll timing' logs/nanoclaw.log | tail -40                      # cycleMs stops reaching ~28 s
```

Gate 2 is met by #315/#324, not by anything a storage-maintenance change could do.
...
- **None that change behavior.** Every duty keeps its cadence, thresholds and log lines, and nothing an agent or a channel sees is different. Through PR 14 there is no migration and no schema change; S2-PR15 adds one nullable column the sweep alone uses. This series is structural.
...
| D8 | Deploy conventions are seam 1's verbatim: one PR per restart, quiet rule v2 re-checked at execution, QA seat first, the five-part gate, revert-build-restart rollback, and no migrations through PR 14 (S2-PR15's 068 is additive, nullable and never read by the reverted code, so rollback stays code-only). Log gates use the two distinct strings `Host sweep duty failed` and `Host sweep mailbox unopenable`, never a shared count (constraint 17). |
