...
| 7 | Delete `src/db/session-db.ts` façade, `session-manager.ts` raw wrappers, and the module's transitional `legacyInboundHandle()`/`legacyOutboundHandle()` bridge (PR 5); port upstream's host `mailbox/registry.test.ts`; host allowlist → empty except module-internal SQLite probes that are not seam consumers by design (`storage-manager.ts` worker-thread reclaim probe over an injected sessions root — documented in PR 5) | host | none | worker | 3–6 |
