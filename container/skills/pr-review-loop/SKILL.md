...
1. After opening the PR, run `codex-review.sh scope`. It waits for the `Risk label` run on the current head (`CODEX_REVIEW_SCOPE_TIMEOUT_SECONDS`, default 300) and prints a `verdict`. It fails closed: a run that is missing, late, failed, or only on an older head gives `review`, never `skip`. So does a `risk:high` or `review:requested` label that anyone but the labeler (`github-actions[bot]`) took off, and label events it cannot read.
