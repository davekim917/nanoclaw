...
- **`resolveSession`, the `sessions` partial UNIQUE indexes.** No change to session resolution or lookup semantics, so critic finding B2 stays dormant exactly as it did for seam 1. Central-DB migrations: zero through PR 14; S2-PR15 adds one additive nullable column (065) and touches no index and no lookup path.
...
- **`quietSessions`** (`:121`) stays in `host-sweep.ts` as fan-out machinery: bounded at `sessions.length + 500`, invalidated by `session.last_active`, hint computed after the last phase. It has a time bound and `last_active` invalidation but deliberately **no mtime change-signal** — a re-home keeping only the mtime half loses the invalidation. **Amended by S2-PR15 (#320, 2026-09-03):** the time bound is jittered per session (deterministic, hash of the id) so the herd does not expire on one 30-minute grid, and the mark is persisted in `sessions.sweep_quiet_until` and warmed at `startHostSweep`, so the first tick after a boot does not scan every active session; the `last_active` invalidation still wins over a persisted mark. No new duty (marks live on session rows; nothing to prune). **As built:** the jitter band is [0.5, 1) x the bound, so it only ever SHORTENS a skip and the 30-minute ceiling above still holds for every session; the persisted column is migration 065 and the `last_active` invalidation wins because `updateSession` nulls it in the same statement that writes `last_active`, which is the durable half of the in-memory `mark.lastActive === session.last_active` check; the never-past-`getNextFutureProcessAfter` bound and the `sessions.length + 500` bound are untouched; and `skipUnreadable`'s backoff is deliberately NOT persisted, because its causes are process-local and a restart is the event that clears them. **Codex round 1 (2026-09-03) added two conditions to that story.** (a) The batch is flushed after the WHOLE fan-out and the driver yields between sessions, so a mark taken early can be invalidated by ingress before it is written; each queued mark therefore carries the `last_active` it was computed against and the UPDATE compares it null-safely (`sessions.last_active IS <basis>`), making the write a no-op for exactly the rows that moved — without it a stale expiry is put back over the clear and a restart warms it. (b) `updateSession` is the only writer of `sessions.last_active`, but it is NOT the only writer of *due-ness*: `dashboard/api/scheduled-mutations.ts` moves `process_after` straight into the session DB on a cron edit and on a resume, and `db/scheduled-tasks.ts`'s `scheduleTask` inserts a task row into a session that may have been quiet for days (`scheduled-move`'s re-home). Both now call `touchSessionActivity`, which is the central-DB write the mark keys on — in `afterMutation` and at the end of `scheduleTask` respectively, both outside any session/funnel callback so the hook survives mailbox PR 7's rewrite of the mutations module.
...
| D8 | Deploy conventions are seam 1's verbatim: one PR per restart, quiet rule v2 re-checked at execution, QA seat first, the five-part gate, revert-build-restart rollback, and no migrations through PR 14 (S2-PR15's 065 is additive, nullable and never read by the reverted code, so rollback stays code-only). Log gates use the two distinct strings `Host sweep duty failed` and `Host sweep mailbox unopenable`, never a shared count (constraint 17). |
