...
- [BREAKING] **The host runtime now requires Node.js 22.19 or newer.** The setup installer upgrades an existing unsupported Node instead of accepting it, deploys fail before install or restart when the service executable is too old, and CI tests the exact minimum plus Node 24. **Migration:** run `bash setup/install-node.sh`, verify the Node executable in the NanoClaw service's `ExecStart` reports v22.19.0 or newer, then restart the existing dependency set before taking later host dependency updates. Stay on the previous commit and restore the prior Node executable/path if the runtime canary fails.
