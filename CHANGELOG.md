...
- [BREAKING] **Scheduled tasks no longer have their own model/effort default.** An unpinned scheduled-task fire on a Claude group used to be forced onto `sonnet` at `xhigh` effort, regardless of what the group was configured to run; it now resolves exactly like interactive chat — the group's configured model and effort, else the provider's per-family default. A task's OWN `--model`/`--effort` pin is unaffected and still wins. Codex and OpenCode were never in this branch. **Who is affected:** any Claude group with *unpinned* scheduled tasks. On the reference install that was 31 of 46 armed series, most moving to `claude-opus-5[1m]` at `high` — a behavior AND per-fire cost increase — while a group whose `container.json` sets its own `model`/`effort` moves to that instead (one group moved to `claude-fable-5-1[1m]` at `medium`, not Opus). Expect the exact count to differ on your install and to drift as tasks are created; the shape is what is stable — every *unpinned* series moves to its group's default. **See what changes before you upgrade:** `ncl tasks list` now shows a `PIN` column, so unpinned series are the rows reading `-`. **To preserve the old behavior, deliberately:** pin the series you care about — `ncl tasks update --id <series> --model sonnet --effort xhigh`. (`ncl tasks repin`, also new in this release, *retargets* pins that already exist, so it is the tool for a later model bump rather than for this migration.) Nothing is auto-pinned on upgrade, on purpose: a pin that changes by itself is not a pin, and silently freezing every unpinned series onto a model nobody chose is the same class of defect this release exists to close.
