...
export function buildSessionServicesSnapshotFrom(
  ...
  if (hasSlackSecret) {
    ...
    if (sessionKnown && !ownerSafe) {
      ...
      useFor =
        'LIVE in THIS session. All Slack read/write is the Web API: `curl https://slack.com/api/<method>` with NO auth header — the OneCLI gateway injects the owner’s user token. There is no Slack MCP; never set your own `Authorization` header or ask for a token (the gateway overwrites it, so a 401 does NOT mean the credential is missing). Methods: `auth.test`, `conversations.history`, `conversations.replies`, `search.messages`, `users.info`, `conversations.list`; `chat.postMessage` posts AS THE OWNER, so only when they ask (POST JSON `{"channel","text","thread_ts"}`). ' +
        'PERMALINKS: `…/archives/C0123/p1789080120758779` → channel `C0123`, ts `1789080120.758779` (dot before the last 6 digits); read with `conversations.replies?channel=C0123&ts=<the link’s thread_ts if present, else that ts>`. ' +
        'FILES: `url_private` bytes live on `files.slack.com`, a SEPARATE credential. Try `curl -sSL -o <path> "<url_private>"`, no auth header, then check it: `text/html` or a leading `<!DOCTYPE` is Slack’s login page — that host is not wired here; say so and stop. Operator fix: a second vault entry, same token, host `files.slack.com`, path `*`, with `slack` and `user` in its name. ' +
        archive +
      ...
      useFor =
        'Scoped to owner-safe Slack contexts (NOT `session_mode`): the owner’s 1:1 DM or a messaging group in `slack_user_token.also_allowed_in`; withheld everywhere else. Where present, all Slack read/write is `curl https://slack.com/api/<method>` with NO auth header (the OneCLI gateway injects the user token) — `conversations.history`/`conversations.replies` for a permalink (`p1789080120758779` → ts `1789080120.758779`), `search.messages`, `users.info`, `chat.postMessage`. There is no Slack MCP. ' +
        archive +
