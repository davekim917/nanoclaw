...
Use only the `shell` tool identity, make at most 12 tool calls, and do not modify the repository. Run at least one of Graphify's public `query`, `path`, `explain`, or `affected` commands through `shell` from `repo/`, then read the named authoritative source files with ordinary shell commands before answering. Do not invoke private Graphify modules or select graph/cache/output paths.
