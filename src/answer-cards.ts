 ...
 *    losing click leaves the card exactly as it was;
...
/** Whether the card behind this question id is a pending answer card. */
export async function isAnswerCard(questionId: string): Promise<boolean> {
  const approval = await getPendingApproval(questionId);
  return approval !== undefined && isAnswerCardAction(approval.action);
}
