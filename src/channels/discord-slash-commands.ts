...
const UPDATE_CONTAINER_PROMPT = [
  ...
  '7. Verify the PR landed on the fork: `gh pr view <#> --repo davekim917/nanoclaw --json baseRepository -q .baseRepository.nameWithOwner` must print `davekim917/nanoclaw`. If it shows anything else, close the PR (`gh pr close <#> --repo <wrong-repo>`) and re-run step 6.',
  '8. STOP. Report the PR URL back to the chat. Do NOT run `gh pr merge` — Dave reviews and merges manually via the GitHub UI.',
  '',
  ...
  '11. Verify the PR landed on the fork: `gh pr view <#> --repo davekim917/bootstrap --json baseRepository -q .baseRepository.nameWithOwner` must print `davekim917/bootstrap`. If it shows anything else, close the PR (`gh pr close <#> --repo <wrong-repo>`) and re-run step 10.',
