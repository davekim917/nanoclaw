...
> **Requires the provisioning port (tracked separately).** The apply steps
> below are upstream's and are not modified. One thing they need is not
> installed here yet — `src/channels/slack-a2a-guard.ts` — so step 1's
> precondition gate stops the install rather than half-applying it. See
> [FORK-NOTES.md](FORK-NOTES.md) for the dependency, and for what installing
> the guard would change about bot-authored traffic fork-wide.
