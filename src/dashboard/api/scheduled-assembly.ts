...
export interface HealthCtx {
  ...
  isOneOff: boolean;
  /**
   * The owning group's effective timezone. The cadence interval is derived
   * from the cron grid, and around a DST transition that interval is 23 or 25
   * hours — so a group whose override transitions on a different date than the
   * install would be measured against the wrong cadence and flip between
   * `late` and `stalled` at the wrong moment. Absent = the install timezone.
   */
  timezone?: string;
}
 ...
 * one-offs (no cron).
 */
function cronIntervalMs(cron: string | null, tz: string = TIMEZONE): number | null {
  if (!cron) return null;
  try {
...
export function deriveHealth(ctx: HealthCtx): HealthState {
  ...
  const interval = ctx.isOneOff ? 2 * SWEEP_INTERVAL_MS : cronIntervalMs(ctx.recurrence, ctx.timezone);
