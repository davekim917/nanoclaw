...
- **The barrier's answer.** On every `lanes` and `synthesis` fire the
  ...
  (`smoke-campaign-controller.py:2139` `publish_barrier`, whose effect writes at
  `:1552-1553` and unlinks on `doc is None` at `:1538-1546`), and every
  barrier-backed brief names that file (`:1573-1596`). `invalid[]` is artifact
  ...
  **A refusal that appears AFTER the brief was acknowledged re-offers the
  ...
  while the `.ack` is absent (`smoke-campaign-controller.py:2711-2713`), so that
  ...
  `briefedRefusal` at `:1491-1492`, carried forward at `:2704-2705`, compared by
  `_reoffer_on_new_refusal` at `:2845`) — and
  ...
  measured from the obligation's first record (`smoke-campaign-controller.py:2722`), so an owner that keeps producing
  ...
  **A refusing barrier wakes the owner on BOTH phases.** By the time a run
  ...
  without a wake (it now wakes at `smoke-campaign-controller.py:3410-3412`),
  ...
  branch declined to create (`_maybe_synthesis_overdue_blocked`, `:3454`,
  reading that obligation at `:3460-3461`), so it could not fire either.
