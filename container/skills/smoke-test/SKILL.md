...
- **The barrier's answer.** On every `lanes` and `synthesis` fire the
  ...
  (`smoke-campaign-controller.py:1830` `publish_barrier`, whose effect writes at
  `:1375-1376` and unlinks on `doc is None` at `:1361-1369`), and every
  barrier-backed brief names that file (`:1396-1419`). `invalid[]` is artifact
  ...
  **A refusal that appears AFTER the brief was acknowledged re-offers the
  ...
  while the `.ack` is absent (`smoke-campaign-controller.py:2337-2339`), so that
  ...
  `briefedRefusal` at `:1314-1315`, carried forward at `:2330-2331`, compared by
  `_reoffer_on_new_refusal` at `:2471`) — and
  ...
  measured from the obligation's first record (`smoke-campaign-controller.py:2348`), so an owner that keeps producing
  ...
  **A refusing barrier wakes the owner on BOTH phases.** By the time a run
  ...
  without a wake (it now wakes at `smoke-campaign-controller.py:2966-2968`),
