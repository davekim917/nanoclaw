...
- **Display**: anything shown to an agent or user renders in the install timezone via `formatLocalTime`/`formatLocalStamp` (`src/timezone.ts`, runner `timezone.ts`); `--json` and DB values stay ISO. Host log lines (`logs/nanoclaw*.log`) are stamped in the service's local `TZ` (`ts()`, `src/log.ts`), not UTC — convert a UTC window before grepping, or the grep matches nothing and reads as clean.
