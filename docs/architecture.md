...
- **Change signal** — session DBs use `journal_mode=DELETE`, so every commit lands in the main file
  and moves its mtime. One `stat` answers "could this session have work?". This would _not_ be safe
...
**Approval routing:** Privilege is a user-level concept. `user_roles` records `owner` (global only — first user to pair becomes owner) and `admin` (global or scoped to a specific `agent_group_id`). When an action requires approval, `pickApprover(agentGroupId)` returns candidates in order: scoped admins for that agent group → global admins → owners (deduplicated). `pickApprovalDelivery` then takes the first candidate reachable via `ensureUserDm` (with a same-channel-kind tie-break so a Discord approval request prefers a Discord-using approver). That tie-break asks `resolveUserChannelType` for the candidate's kind rather than splitting the user id on its own `:` prefix — Teams ids are `29:<aad-id>`, so the prefix is not the channel type and a prefix comparison would drop the only reachable approver. The approval card lands in the approver's DM messaging group, not the origin chat. Delivery is resolved through the Chat SDK's `openDM` for resolution-required channels (Discord/Slack/…) or the user's handle directly for direct-addressable channels (Telegram/WhatsApp/…), and the mapping is cached in `user_dms` for subsequent requests. See `src/modules/permissions/access.ts` and `src/modules/permissions/user-dm.ts` (`ensureUserDm`); the approver-picking primitives live in `src/modules/approvals/primitive.ts`.
...
- **`task`** — run the pre-script if present, then format as `<task from="…" time="…" current_time="…">` (script output first). `time` is the occurrence's scheduled slot (`process_after`); `current_time` is when the run actually reached the agent.
