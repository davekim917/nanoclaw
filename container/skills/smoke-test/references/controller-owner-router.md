...
3. Do only that step. Write only the artifacts the brief names, under
   ...
   every brief it writes (`smoke-campaign-controller.py:1466-1468`, in
   `_owner_wake` at `:1411`), which is why the file is current and your own
   ...
   - Your ack does not carry over. Writing a brief removes
     ...
     (`smoke-campaign-controller.py:1455-1465`), so a re-offered step needs a
