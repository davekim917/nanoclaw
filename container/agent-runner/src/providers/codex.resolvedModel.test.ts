...
describe('non-Claude providers name the model they resolved', () => {
  it('codex falls back to its own configured model, never to absence', () => {
    // codex's chain is stickyConfig.model ?? CODEX_MODEL ?? 'gpt-5.6-terra', and
    // resolveQueryModel is the function the query path uses to apply it.
    expect(resolveQueryModel(undefined, 'gpt-5.6-terra')).toBe('gpt-5.6-terra');
    expect(resolveQueryModel('gpt-6-astra', 'gpt-5.6-terra')).toBe('gpt-6-astra');
    // A non-codex model is refused and the fallback still NAMES something.
    expect(resolveQueryModel('claude-sonnet-5', 'gpt-5.6-terra')).toBe('gpt-5.6-terra');
  });
