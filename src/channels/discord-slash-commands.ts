...
export const UPDATE_CONTAINER_PROMPT = [
  ...
  "Before proposing any host/container bump, check whether the most recent upstream merge RESOLVED that dependency as keep-ours: `git log --merges -1 --format=%H upstream/main..HEAD` is not the test — find the last `Merge remote-tracking branch 'upstream/main'` commit M, then compare `git show M^1:<manifest>`, `git show M^2:<manifest>`, and `git show M:<manifest>`. If the merge kept OUR pin over upstream's, that is a standing decision: report it, and do not propose moving past it without saying so explicitly and asking. #135 bumped `@onecli-sh/sdk` ^0.5.0 -> ^2.8.0 five hours after merge ceb3fcd1 had deliberately kept ^0.5.0 over upstream's 2.2.1, and nothing connected the two. This check is stronger than upstream parity, which would NOT have caught it — upstream 2.2.1 is also incompatible with our gateway.",
