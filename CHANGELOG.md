...
- [BREAKING] **Minimum supported Node.js raised to 22** (was 20), matching the host runtime. `engines.node`, CI, and docs updated to match; `.nvmrc` and `setup/install-node.sh` already installed 22, and the bootstrap's version gate (`setup.sh`) plus `setup/install-node.sh`'s already-installed check now both enforce the new floor instead of accepting any Node ≥20. **Migration:** installs still on Node 20/21 must upgrade before continuing — re-run `bash nanoclaw.sh` (its bootstrap step now installs Node 22 automatically) or run `setup/install-node.sh` directly, then restart the service; confirm with `node --version` reporting `v22.x`.
