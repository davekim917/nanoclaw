...
- **Memory or standing instructions** → Edit `memory/` or `standing-instructions.md` directly, no approval needed. The workspace is persisted on the host. The composed provider document (`CLAUDE.md` or `AGENTS.md`) is regenerated every spawn and must not be edited.
