...
`messaging_group_agents.instructions_profile` carries the **operating rules for one channel** — what the agent may touch in this room, whether it may ask questions, which repos are in bounds. It is a separate always-on layer from `default_tone`, and the split is the point: **tone stays voice-only**, one slot, by invariant. Rules and register are different axes, and folding a rule set into a tone profile makes the voice slot arbitrate two unrelated concerns.

- **Where the files live**: `groups/<folder>/channel-instructions/<name>.md`, in the private groups repo. The host binds each `.md` read-only at `/workspace/channel-instructions/<name>.md`.
- **Sharing between siblings**: symlink the file (or the whole directory) at a workgroup sibling's copy — `groups/<sibling>/channel-instructions/<name>.md -> ../../<owner>/channel-instructions/<name>.md`. The host resolves the link **host-side** at spawn and binds the real file, the same way `standing-instructions.md` is shared (`readGroupPersona`). A link resolving outside the group's own directory, a workgroup sibling's, or the workgroup shared tree is refused with a warning and nothing is bound — the group directory is mounted read-write, so an unchecked link would be a way for an agent to write its own always-on prompt.
- **Why file-by-file mounts**: `/workspace/channel-instructions` sits at a different depth from `groups/<folder>/channel-instructions`, so a *relative* per-file symlink resolves to a different place on each side and would dangle inside the container. Binding resolved real files at literal destinations avoids that entirely (`channelInstructionsMounts` in `src/container-runner.ts`).
- **Resolution**: per-wiring or nothing. The host reads the column next to `default_tone` and forwards `NANOCLAW_INSTRUCTIONS_PROFILE`; there is **no `container.json` fallback**, because the group-wide equivalent is `standing-instructions.md`, already in every prompt. A second group-level slot would only be a way for the two to disagree.
- **Injection**: the runner loads the file and places the block **ahead of the tone block** in `systemContext.instructions` (`container/agent-runner/src/index.ts`), under a `# Channel instructions (<name>)` heading, so operating rules are read before voice. All three providers (Claude, Codex, OpenCode) read `systemContext.instructions`, so the block reaches every one. A missing or unreadable file logs a warning and the session continues without it — a wiring pointing at a renamed profile must still answer.
- **Setting it**: `ncl wirings create|update --instructions-profile <name>`; `--instructions-profile ""` clears it back to NULL. Names must match `^[a-z0-9][a-z0-9-]*$`, rejected at the CLI and re-checked in the runner (the name indexes a filename and travels through the container environment).
- **Not inherited.** Auto-wire and the workspace-trust inherit path both write NULL. Tone is inherited across a platform's channels when unanimous; instructions are the rules of one specific room, so carrying them into a new room would silently extend a scoped rule set past its scope.
