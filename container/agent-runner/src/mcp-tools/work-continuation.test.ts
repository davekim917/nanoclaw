...
describe('work continuation tools', () => {
  it('continue_work persists a validated queued task', async () => {
    ...
    const row = getOutboundDb()
      .prepare("SELECT value FROM session_state WHERE key = 'work_continuation'")
      .get() as { value: string };
    expect(JSON.parse(row.value)).toMatchObject({ task: 'write focused tests', phase: 'queued', chain: 1 });
  });
  ...
  it('rejects empty, oversized, and unknown input without writing', async () => {
    ...
    expect(
      getOutboundDb().prepare("SELECT 1 FROM session_state WHERE key = 'work_continuation'").get(),
    ).toBeNull();
  });
