...
A second move through the same seam; §1–§5 below are written for the current values. `DEFAULT_OPUS_MODEL` (`src/flag-parser.ts`) is now `claude-opus-5-5[1m]`, and the Opus family default effort (`defaultEffortForModel` in `container/agent-runner/src/providers/claude.ts`) is now `medium`, down from `high`. For this move:
...
Before this change that prints the group's model in `ANTHROPIC_DEFAULT_OPUS_MODEL`; after it, the install's Opus constant in every group (`claude-opus-5-5[1m]` since 2026-09-22), plus a separate `NANOCLAW_CLAUDE_MODEL` holding the group's model.
...
Expect `ANTHROPIC_DEFAULT_OPUS_MODEL=claude-opus-5-5[1m]` in **every** Claude group, and `NANOCLAW_CLAUDE_MODEL` equal to that group's pin (or `claude-opus-5-5[1m]` when unpinned). A container still showing `claude-opus-5[1m]` is running a pre-2026-09-22 build or image.
