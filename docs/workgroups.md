...
HEAD and the stash are already on `origin`. A tag the clone inherited
unchanged from the canonical (same name, same object) is the canonical's, not
the clone's own work, so it does not count.
