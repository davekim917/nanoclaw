...
Use only the `shell` tool identity, make at most 12 tool calls, and do not modify the repository. Run at least one actual Graphify `query`, `path`, `explain`, or `affected` read through `shell` from `repo/`, then read the named authoritative source files with ordinary shell commands before answering. `graphify --help` and `graphify <command> --help` are side-effect-free syntax summaries but do not satisfy the required graph read. Do not invoke private Graphify modules or select graph/cache/output paths.
