...
`scripts/precheck-opencode.sh --pr <n> [--repo <owner/name>] [--model <id>]` reads the PR body and diff and asks the local OpenCode CLI whether the diff does what the body claims, whether any number/path/sha/test count in the body is contradicted, whether a changed test was weakened to make something pass, and what second site or regenerated baseline is missing. It runs on `opencode/deepseek-v4.1-flash` by default, on a capacity pool separate from Claude and Codex, so a wrong claim gets caught before a scarce reviewer round starts from a wrong PR body.

It is a **pre-check, not a review**: it writes no receipt, and the merge gate neither reads nor honours its output — `codex-review.sh receipt` remains the only thing that satisfies the gate. Its model happens to be gate-eligible, so its findings *can* be promoted into a receipt, but only by deliberately calling `receipt --reviewer "deepseek-v4.1-flash …"`; running the pre-check never implies one. A non-zero exit (rate limit, timeout) is not a pass, and neither is a zero exit with no output — the script captures the run and refuses an empty result rather than letting a blank terminal read as "no findings". Because the output is captured, nothing appears until the run ends.
...
`request` refuses, posting nothing, with 20 (not risk-scoped), 21 (verdict `skip`), 22 (this head already requested), 23 (cap), or 3 (churn gate). When `wait` exits 11 or 13, run the substitute review [Review availability](../../../docs/review-policy.md#review-availability) requires — a fresh-context reviewer, never the implementing session — and post it with `codex-review.sh receipt`, whatever the verdict: `changes` too. Reviewer eligibility is one frontier model per provider: `claude-opus-5` on Claude and `gpt-5.6-sol` on Codex, both at `high` effort, and `deepseek-v4.1-flash` on OpenCode (`opencode run -m opencode/deepseek-v4.1-flash`, a third capacity pool for when both of the others are rate-limited; `opencode run` has no effort flag, so there the id is the whole tier). An explicit effort override must use a runtime field or scoped CLI invocation, never prompt wording. Prior Fable/Astra reviewer IDs remain accepted for receipt compatibility, so an unchanged exact-head approval survives roster migration; they are not dispatch defaults. The allowed ids are generated: `container/skills/pr-review-loop/reviewer-models.txt` (`scripts/reviewer-models.ts --write`), and `receipt`/`merge-check` check only `--reviewer`'s **first whitespace-delimited word** against it (a `[1m]` suffix on that word is tolerated) — `--reviewer` not starting with an allowed id is refused before it posts. Everything after that first word is free text, so a receipt written under an older parenthetical (`claude-opus-5 (worker-frontier)`) still unlocks the head it was posted against. Report the reviewer's **exact model id from its own runtime** — a Claude subagent from its system prompt, Codex from the `-m` it ran with or `codex exec`'s session metadata — as that first word, e.g. `claude-opus-5 (opus)`, `gpt-5.6-sol high (codex exec)` or `deepseek-v4.1-flash (opencode run)` — the id is stored bare, without the `opencode/` provider prefix, because `assertConcreteModelId` refuses `/`. Don't wait for a reviewer to be free: start one yourself as a fresh process, `codex exec -m gpt-5.6-sol -c model_reasoning_effort=high`, `CLAUDE_CODE_EFFORT_LEVEL=high claude -p --model 'claude-opus-5[1m]' --effort high` or `opencode run -m opencode/deepseek-v4.1-flash '<prompt>' < /dev/null` (the redirect is load-bearing — an open stdin makes the CLI wait for more prompt input and hang), give it the head SHA, the complete diff, the relevant files and the review policy, and post its report as the receipt body. The latest receipt for that exact head decides: `approve` satisfies the review, and `changes` refuses the merge under either verdict — even over a clean Codex review — until a later `approve`. A receipt posted before this allowlist gate existed, whose `--reviewer` doesn't START with an allowed id, no longer unlocks a merge — re-post a fresh receipt with an allowlisted `--reviewer` to merge through the new gate.
