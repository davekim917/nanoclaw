...
**A head that moved only by bookkeeping needs a quick re-receipt, not a new review or a CI-full rerun.** A receipt covers exactly one head, so any move needs a new one. The quick path applies only when the new head is the approved head with the base merged in and nothing else changed except a regenerated `src/upstream-ratchet.json` and/or `docs/review-notes/` fragments. Prove it with an exact check of the merge result, not with range-diff, which never inspects the merge commit itself:

```bash
OLD=<approved head>  NEW=<new head>  BASE=<the base commit that was merged in: NEW's second parent>
T=$(git merge-tree --write-tree "$OLD" "$BASE") || T=$(git merge-tree --write-tree "$OLD" "$BASE" | head -1)
git diff --name-only "$T" "$NEW" | grep -v -E '^(src/upstream-ratchet\.json|docs/review-notes/[^/]+\.md)$'
```

`T` is the tree git's automatic merge of the approved head and the base produces. The last command must print nothing. A hand-resolved conflict in any other file differs from `T`, so it prints too. Any other line means the move changed code, or an edit was hidden in the merge commit, and that needs a full review. The PR's `CI` workflow must also be green on the new head, because it runs the ratchet and review-notes tests that catch a stale hash or a bad citation. Paste the three command results into the receipt body. Don't re-run ci-full for that move: the gate requires only the workflows in `CODEX_REVIEW_REQUIRED_WORKFLOWS`, default `CI` (the `ci_status` comment in `scripts/codex-review.sh`). Re-run ci-full, and review in full, whenever the check prints a file.
