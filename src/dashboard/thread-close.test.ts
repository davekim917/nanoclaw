...
// Point the session-DB opens at a scratch tree so the wrap-up write and the
// REAL force-clear run against real SQLite files, not stubs. Everything else in
// session-manager passes through.
const SESSIONS_ROOT = fs.mkdtempSync(path.join(os.tmpdir(), 'nc-thread-close-'));
const dbPathFor = (agentGroupId: string, sessionId: string, file: string) =>
  path.join(SESSIONS_ROOT, agentGroupId, sessionId, file);

vi.mock('../session-manager.js', async (importActual) => {
  const actual = await importActual<typeof import('../session-manager.js')>();
  const sessionDb = await import('../db/session-db.js');
  return {
    ...actual,
    openInboundDb: (agentGroupId: string, sessionId: string) =>
      sessionDb.openInboundDb(dbPathFor(agentGroupId, sessionId, 'inbound.db')),
    openOutboundDb: (agentGroupId: string, sessionId: string) =>
      sessionDb.openOutboundDbRw(dbPathFor(agentGroupId, sessionId, 'outbound.db')),
    openOutboundDbRw: (agentGroupId: string, sessionId: string) =>
      sessionDb.openOutboundDbRw(dbPathFor(agentGroupId, sessionId, 'outbound.db')),
    outboundDbPath: (agentGroupId: string, sessionId: string) => dbPathFor(agentGroupId, sessionId, 'outbound.db'),
  };
});
...
function materializeSession(agentGroupId: string, sessionId: string): void {
  fs.mkdirSync(path.join(SESSIONS_ROOT, agentGroupId, sessionId), { recursive: true });
...
import {
  ...
  _forceClearWorkContinuationForTesting,
...
describe('requestThreadClose', () => {
  ...
  it('the wrap-up asks for the confirmation and names the deadline', () => {
...
describe('decideCloseFinalization', () => {
  ...
  it('waits while the agent has not answered and the window is open', () => {
    expect(decideCloseFinalization({ requestedAtMs, now: NOW, proposalAtMs: [null] })).toEqual({ finalize: false });
  });

  it('a proposal that predates the request is not an answer to it', () => {
  ...
  it('finalizes unforced once every session has answered', () => {
  ...
  it('finalizes forced once the bounded window elapses, and only then', () => {
...
describe('the close sequence order', () => {
  ...
  it('clears the continuation BEFORE killing, and archives only after the process is gone', () => {
    startClose();
    const { calls, deps } = recordingDeps();
    advanceThreadClosures(deps);
  ...
  it('does NOT kill or archive when the continuation cannot be cleared', () => {
    startClose();
  ...
  it('skips the kill for a session with no container but still clears and archives', () => {
    startClose();
  ...
  it('archives as the TERMINAL marker and marks the closure closed and forced', () => {
    startClose();
  ...
  it('finalizes unforced when the agent confirms, without waiting out the window', () => {
  ...
  it('does nothing at all while the agent still has time to answer', () => {
    startClose({ requestedAt: iso(30_000) });
    const { calls, deps } = recordingDeps();
    advanceThreadClosures(deps);
  ...
  it('end to end: the REAL clear empties the session DB before the container is killed', () => {
    startClose();
    materializeSession('ag1', 's1');
  ...
  it('is idempotent — a second tick over an already-closed thread does nothing', () => {
    startClose();
    advanceThreadClosures(recordingDeps().deps);
    const second = recordingDeps();
    advanceThreadClosures(second.deps);
...
describe('force-clearing a work_continuation', () => {
  it('drops both the current and the legacy continuation keys', () => {
    ...
    _forceClearWorkContinuationForTesting(db, { threadId: 't', sessionId: 's1' });
  ...
  it('clears a record it cannot parse — presence, not validity, is the question', () => {
    const db = outboundStub();
    db.prepare('INSERT INTO session_state (key, value, updated_at) VALUES (?, ?, ?)').run(
      'work_continuation',
      'not json',
...
describe('readDoneProposal / syncDoneProposalMirror', () => {
  ...
  it('reads a valid proposal and rejects everything else as absent', () => {
  ...
  it('mirrors onto the session row, and clears it when the agent retracts', () => {
    ...
    syncDoneProposalMirror('s1', outboundStub());
