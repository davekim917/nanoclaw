...
3. Do only that step. Write only the artifacts the brief names, under
   ...
   every brief it writes (`smoke-campaign-controller.py:1455-1457`, in
   `_owner_wake` at `:1400`), which is why the file is current and your own
   ...
   - Your ack does not carry over. Writing a brief removes
     ...
     (`smoke-campaign-controller.py:1444-1454`), so a re-offered step needs a
