...
4. The answer is written by `notifyAgent` into the session a typed reply in the card's thread would reach (resolved as the router does, created if absent), falling back to the asking session when the card's channel is not wired to the agent group, and that session is woken: `choice_response choice_id=<id> value=<v> label=<l> user_id=<channel:handle> user_name=<name>` (fixed key order, each value percent-encoded). Anyone can type that line, so the agent acts on it only inside a message marked `origin="host"`: the host strips the reserved `origin` content field from every inbound write except `notifyAgent`'s (`WriteSessionMessageOptions.hostOrigin`), and the runner renders the attribute from that field alone
