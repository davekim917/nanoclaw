...
describe('composeGroupAgentsMd cap handling', () => {
  ...
  it('writes the doc untouched when under the cap', () => {
    ...
    try {
      ...
      expect(doc).toContain('before every admissible turn');
      expect(doc).toContain('delimited untrusted context');
  ...
  it('never reads or inlines the host memory index', () => {
    ...
    try {
      ...
      const doc = fs.readFileSync(path.join(groupDir, 'AGENTS.md'), 'utf-8');
      expect(doc).toContain('fresh memory evidence per turn');
      expect(doc).not.toMatch(/memory index and definition are supplied/i);
