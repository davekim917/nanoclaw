 ...
 *
 * Three nested bind mounts, ordered by specificity. Docker applies mount
 ...
 *      Keeps skills, agents, settings.json, plugins, statsig, and
 *      shell-snapshots group-scoped (initialized once by initGroupFilesystem,
 *      read-mostly at runtime, races are benign).
 *
 *   2. Per-session `projects/<hash>/` → `/home/node/.claude/projects/<hash>/`
 ...
 *      fresh Claude session instead of reloading prior context. (v1 avoided
 *      this by isolating the entire `.claude` dir per-thread; v2 isolates
 *      only the projects dir and keeps the rest group-shared so skills/
 *      agents don't have to be copied per-session.)
 *
 *   3. Group-shared `memory/` → `/home/node/.claude/projects/<hash>/memory`
