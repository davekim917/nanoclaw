...
const UPDATE_CONTAINER_PROMPT = [
  ...
  '8. HOST PARITY (required whenever the bumped set includes `@openai/codex` / the `CODEX_VERSION` ARG): the host runs its own npm-global codex that shares `~/.codex` with every container — version skew between them poisons the shared model-list cache (models_cache.json) and hides new models from the host picker. The PR body MUST end with this checklist so the merge reminder survives review:',
  '   - [ ] Host parity — run on the host after merge: `npm i -g @openai/codex@<new version>` then `mv ~/.codex/models_cache.json /tmp/`',
