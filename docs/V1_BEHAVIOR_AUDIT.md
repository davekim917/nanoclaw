...
| 13 | v1 container-runner `groupMemoryDir` bind over `.claude/projects/.../memory` | Cross-thread auto-memory overlay within an agent group | **REGRESSED / UNVERIFIED** — v2 `.claude-shared` is per-agent-group but no nested-overlay; cwd-derived memory path may land isolated per session | HIGH | 4 | Live-verify that memory written in session A is visible in session B of the same agent group. If not, add overlay mount. **Same class as the worktree-autosave miss.** |
...
| 20 | v1 `src/channels/slack.ts` native adapter — reply-context extraction on inbound | Agent sees "replying to <msg>" context in Slack | **PARTIAL** — v2 `slack.ts` does NOT pass `extractReplyContext` to chat-sdk bridge; Discord does | HIGH | 4 | Add extractReplyContext in v2 slack.ts. Small diff, same shape as Discord. |
| 21 | v1 Phase 2.9 chat-sdk outbound archive filter | Every outbound message archived | v2 `delivery.ts:461` archives only `kind==='chat'`. **If chat-sdk adapters (Slack+Discord) deliver `kind==='chat-sdk'`, agent replies never get archived** | HIGH (pending delivery-kind trace) | 4 | Trace `delivery.ts` to confirm whether chat-sdk channels translate to `kind:'chat'` on outbound. If not, extend filter. |
...
| 26 | v1 `a9314b5` model drift detection | When `[1m]` suffix silently dropped mid-session, force `setModel('sonnet[1m]')` | **MISSING** — v2 has no model tracking at all | MEDIUM (dependent on model-switch port #7) | 5 | Skip until #7 lands. |
| 27 | v1 `276bad3` `/compact` model re-assert | Re-invoke setModel after /compact session reset | **NEEDS VERIFY** | HIGH | 5 | Inspect providers/claude.ts /compact handler. |
