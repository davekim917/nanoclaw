...
describe('steer_idempotency DAO', () => {
  it('test_reserve_fresh', async () => {
    const result = await reserveIdempotency('u1', 'key-1', TASK, 'msg-1', 'hello', 'hash-h');
  ...
  it('test_reserve_replay_same_payload', async () => {
    await reserveIdempotency('u1', 'key-1', TASK, 'msg-1', 'hello', 'hash-h');
    const replay = await reserveIdempotency('u1', 'key-1', TASK, 'msg-2', 'hello', 'hash-h');
    expect(replay.messageId).toBe('msg-1');
  });

  it('test_reserve_target_id_conflict', async () => {
    await reserveIdempotency('u1', 'key-1', TASK, 'msg-1', 'hello', 'hash-h');
    await expect(reserveIdempotency('u1', 'key-1', TASK_OTHER, 'msg-2', 'hello', 'hash-h')).rejects.toThrow(
      IdempotencyConflict,
    );
    try {
      await reserveIdempotency('u1', 'key-1', TASK_OTHER, 'msg-2', 'hello', 'hash-h');
    } catch (e) {
  ...
  it('test_reserve_target_type_conflict: task→session reuse same key', async () => {
    await reserveIdempotency('u1', 'key-1', TASK, 'msg-1', 'hello', 'hash-h');
    try {
      await reserveIdempotency('u1', 'key-1', SESSION, 'msg-2', 'hello', 'hash-h');
  ...
  it('test_reserve_request_hash_conflict', async () => {
    await reserveIdempotency('u1', 'key-1', TASK, 'msg-1', 'hello', 'hash-h');
    try {
      await reserveIdempotency('u1', 'key-1', TASK, 'msg-2', 'goodbye', 'hash-OTHER');
    } catch (e) {
      expect(e).toBeInstanceOf(IdempotencyConflict);
  ...
  it('test_apply_advances_status', async () => {
    await reserveIdempotency('u1', 'key-1', TASK, 'msg-1', 'hello', 'hash-h');
    await applyIdempotency('u1', 'key-1', makeResponse());
  ...
  it('test_apply_idempotent', async () => {
    await reserveIdempotency('u1', 'key-1', TASK, 'msg-1', 'hello', 'hash-h');
    await applyIdempotency('u1', 'key-1', makeResponse());
    await expect(applyIdempotency('u1', 'key-1', makeResponse())).resolves.not.toThrow();
  ...
  it('test_markEchoAttempted_sets_flag', async () => {
    const reserved = await reserveIdempotency('u1', 'key-1', TASK, 'msg-1', 'hello', 'hash-h');
    await markEchoAttempted(reserved.id);
  ...
  it('session-targeted reservation persists target_type=session', async () => {
    await reserveIdempotency('u1', 'key-2', SESSION, 'msg-1', 'hi', 'hash-h');
    const row = getRawDb()
