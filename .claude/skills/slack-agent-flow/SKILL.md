...
> below are upstream's and are not modified. Twelve of the things they need are
> not installed here yet, so steps 1 and 2 stop the install at their
> precondition gates rather than half-applying it. See
> [FORK-NOTES.md](FORK-NOTES.md) for the itemized list, produced by running
> those gates term by term against this tree.
