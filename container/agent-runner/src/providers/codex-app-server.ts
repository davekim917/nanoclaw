...
const CODEX_MCP_CHILD_ENV_SH = '/tmp/nanoclaw-codex-mcp-env.sh';
const CODEX_MCP_CHILD_ENV_KEYS = [
  'HTTPS_PROXY',
  'HTTP_PROXY',
  'https_proxy',
  'http_proxy',
...
function writeCodexMcpChildEnvFile(): void {
  const lines = ['# Generated by NanoClaw. Contains container-local proxy/cert env for Codex MCP children.'];
  for (const key of CODEX_MCP_CHILD_ENV_KEYS) {
    const value = process.env[key];
    if (value) lines.push(`export ${key}=${shellQuote(value)}`);
  }
  fs.writeFileSync(CODEX_MCP_CHILD_ENV_SH, `${lines.join('\n')}\n`, { mode: 0o600 });
  fs.chmodSync(CODEX_MCP_CHILD_ENV_SH, 0o600);
}

function asRemoteBridgeServer(name: string, config: CodexMcpServer): CodexMcpServer {
  const args = config.args ?? [];
  if (config.command !== 'bun' || args[0] !== '/app/src/remote-mcp-bridge.ts' || !args[1]) return config;

  // Codex app-server currently starts MCP stdio children with a restricted
  // environment. Source the OneCLI proxy/cert env before Bun starts, and pass
  // bridge config via shell exports so hosted MCPs don't send placeholders
  // directly to the remote server.
  const exports = [`export REMOTE_MCP_NAME=${shellQuote(config.env?.REMOTE_MCP_NAME ?? name)}`];
  const authorization = config.env?.REMOTE_MCP_AUTHORIZATION;
  if (authorization) exports.push(`export REMOTE_MCP_AUTHORIZATION=${shellQuote(authorization)}`);
  ...
  return {
    ...
    args: [
      '-lc',
      [
        `. ${shellQuote(CODEX_MCP_CHILD_ENV_SH)} 2>/dev/null || true`,
        ...exports,
        `exec /usr/local/bin/bun /app/src/remote-mcp-bridge.ts ${shellQuote(args[1])}`,
      ].join('; '),
    ],
  };
}
...
export function writeCodexMcpConfigToml(servers: Record<string, CodexMcpServer>): void {
  ...
  for (const [name, rawConfig] of Object.entries(servers)) {
    const config = asRemoteBridgeServer(name, rawConfig);
