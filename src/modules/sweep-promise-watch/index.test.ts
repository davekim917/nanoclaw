...
function session(id: string, over: Partial<Session> = {}): Session {
  return {
    ...
    last_active: iso(QUIET_MS + 60_000),
...
describe('scanOnce', () => {
  ...
  function deps(over: Partial<ScanDeps> & { snaps?: Record<string, SessionSnapshot> } = {}) {
    ...
    return { d, nudge: (over.nudge as typeof nudge | undefined) ?? nudge, classify };
  }
  ...
  it('skips a session whose DBs cannot be read, and carries on', async () => {
    const { d } = deps({
      snaps: { a: snap(), b: snap({ latestChat: { id: 'm2', timestamp: iso(QUIET_MS + 60_000), text: 'x' } }) },
      snapshot: async (s) => {
...
describe('helpers', () => {
  ...
  it('reads the newest chat row, not a later status edit', () => {
    ...
    db.exec('CREATE TABLE messages_out (id TEXT, seq INTEGER, kind TEXT, timestamp TEXT, content TEXT)');
    const ins = db.prepare('INSERT INTO messages_out VALUES (?, ?, ?, ?, ?)');
    ins.run('c1', 1, 'chat', '2026-09-17T10:00:00Z', '{"text":"first"}');
    ins.run('c2', 3, 'chat', '2026-09-17T11:00:00Z', '{"text":"I will retry"}');
    ins.run('s1', 5, 'status', '2026-09-17T11:05:00Z', '{"text":"working"}');
