...
3) Ticket:
   - `existingTicket` is null (new, passed pre-flight): decide team — **Apollo** if it clearly references Apollo; otherwise **XZO** (XZO is the failover default). Create the issue via `mcp__linear__save_issue` (title = subject; description with From/Date/Subject + full body + `Source: support@illysium.ai · Gmail thread <threadId>`; priority 2/High if urgent/outage/broken/can't-login, else 3/Medium). Append the mapping to the SHARED workflow map at `/workspace/workgroup/support_ticketed_threads.json` (`m['<threadId>'] = {team, issue}`) — REQUIRED so future replies don't double-ticket. (This file is shared across all support-assigned siblings — NOT a private bedroom file — so whoever runs the poller next keeps continuity.)
