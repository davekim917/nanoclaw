...
A declaration may name, in its leading comment block, a context it stands in for — `# host-ci-context: CI Gate` — when a branch rule requires that context by name. run-host-ci.sh then posts that context too, with the same state, and only when every Actions job of that name on the head is one GitHub never started (`never-started.jq`, the same predicate merge-check uses): never over a real Actions result, never while Actions is healthy. `CI (host)`, `Release policy` and `Release approval` cannot be named. It does not make GitHub's rule pass by itself: GitHub requires a same-named check run and commit status to *both* pass, and the never-started check run is still there, failed — so an outage merge still goes through the rule's bypass, after merge-check says `ci=host`.
