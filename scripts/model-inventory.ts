...
for (const s of db
  ...
  try {
    const o = new Database(p, { readonly: true, fileMustExist: true });
    ...
    /* session DB without session_state — nothing sticky */
  }
}
...
const out = { installDefaults, groups: groupRows, wirings, pinned, unpinnedCount, subagents, stickies, observed };
if (asJson) {
