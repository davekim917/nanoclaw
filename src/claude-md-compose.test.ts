...
describe('standing instructions reach every provider identically', () => {
  // Regression that motivated this whole surface: per-group rules used to live
  // in CLAUDE.local.md, which Claude Code auto-discovers and Codex/OpenCode
  // never read, so trust-boundary rules ("never permanently delete an email", a
  // client's "never name AI tooling in these repos") reached one sibling of
  // three. One persona file, composed into both generated docs, is the fix.
  const RULE = 'Never permanently delete an email.';

  for (const provider of ['claude', 'codex', 'opencode']) {
    it(`puts the standing instructions in CLAUDE.md AND AGENTS.md for ${provider}`, async () => {
      const ag = group(`ag-reach-${provider}`, `reach-${provider}`);
      await seed(ag);
      writePersona(ag.folder, `# Group rules\n\n${RULE}\n`);

      await composeGroupClaudeMd(ag, provider);

      const agents = fs.readFileSync(path.join(GROUPS_DIR, ag.folder, 'AGENTS.md'), 'utf-8');
      expect(docOf(ag.folder)).toContain(RULE);
      expect(agents).toContain(RULE);
    });
  }

  // The two generated files are one document: AGENTS.md is CLAUDE.md behind a
  // provenance header, with nothing appended. A tail here is exactly how a rule
  // used to reach some providers and not others.
  it('writes AGENTS.md as CLAUDE.md plus only its header', async () => {
    const ag = group('ag-same-doc', 'same-doc');
    await seed(ag);
    writePersona(ag.folder, `${RULE}\n`);

    await composeGroupClaudeMd(ag, 'codex');

    const agents = fs.readFileSync(path.join(GROUPS_DIR, ag.folder, 'AGENTS.md'), 'utf-8');
    const header = agents.slice(0, agents.indexOf('\n\n') + 2);
    expect(header.startsWith('<!-- Generated by composeGroupClaudeMd')).toBe(true);
    expect(agents.slice(header.length)).toBe(docOf(ag.folder));
  });

  it('does NOT expand @-includes in the standing instructions (host-side exfiltration guard)', async () => {
    ...
    expect(docOf(ag.folder)).not.toContain('SENTINEL_HOST_SECRET_d41d8cd9');
  });
});

describe('legacy CLAUDE.local.md is retired', () => {
  ...
  it('is never created, not even on a first spawn', async () => {
    const ag = group('ag-local-none', 'local-none');
    await seed(ag);

    await composeGroupClaudeMd(ag, 'codex');

    expect(fs.existsSync(localPath(ag.folder))).toBe(false);
  });

  // Earlier builds wrote an empty placeholder on every init and spawn.
  for (const [label, body] of [
    ['empty', ''],
    ['whitespace-only', '  \n\t\n'],
  ] as const) {
    it(`removes an ${label} placeholder without warning`, async () => {
      const ag = group(`ag-local-${label}`, `local-${label}`);
      await seed(ag);
      ...
      expect(fs.existsSync(localPath(ag.folder))).toBe(false);
      expect(vi.mocked(log.warn)).not.toHaveBeenCalledWith(LEGACY_WARN, expect.anything());
    });
  }

  // Real content is an agent's identity. Moving it is the operator's edit, so
  // the file is left exactly as found — and it must NOT leak back into
  // AGENTS.md, which is the one-provider-only reach this retires.
  it('keeps a file with content untouched, warns, and composes none of it', async () => {
    const ag = group('ag-local-content', 'local-content');
    await seed(ag);
    fs.mkdirSync(path.join(GROUPS_DIR, ag.folder), { recursive: true });
    fs.writeFileSync(localPath(ag.folder), 'SENTINEL_LEGACY_LOCAL_7c1e\n');

    await composeGroupClaudeMd(ag, 'codex');

    expect(fs.readFileSync(localPath(ag.folder), 'utf-8')).toBe('SENTINEL_LEGACY_LOCAL_7c1e\n');
    expect(fs.readFileSync(path.join(GROUPS_DIR, ag.folder, 'AGENTS.md'), 'utf-8')).not.toContain(
      'SENTINEL_LEGACY_LOCAL_7c1e',
    );
    expect(docOf(ag.folder)).not.toContain('SENTINEL_LEGACY_LOCAL_7c1e');
    expect(vi.mocked(log.warn)).toHaveBeenCalledWith(LEGACY_WARN, { group: ag.folder, kind: 'file' });
  });

  // A symlink is never followed: removing it would be judging a file this
  // function cannot see, and an EMPTY target is exactly the case where
  // following it and deleting would be wrong — the link is somebody's.
  it('leaves a symlink and its target alone, even when the target is empty', async () => {
    const ag = group('ag-local-link', 'local-link');
    await seed(ag);
    const dir = path.join(GROUPS_DIR, ag.folder);
    fs.mkdirSync(dir, { recursive: true });
    const target = path.join(dir, 'elsewhere.md');
    fs.writeFileSync(target, '');
    fs.symlinkSync('elsewhere.md', localPath(ag.folder));
...
describe('AGENTS.md is never truncated', () => {
  ...
  // reading the file, never by a byte number. A group with oversized standing
  // instructions must still get the FULL doc, for every provider.
  function withHugePersona(folder: string): void {
    // Comfortably over 40KB, as many separate sections.
    writePersona(folder, Array.from({ length: 45 }, (_, i) => `## Filler ${i}\n\n${'x'.repeat(1000)}`).join('\n\n'));
  }
  ...
  for (const provider of ['codex', 'opencode', 'claude']) {
    it(`does not truncate the document for ${provider}`, async () => {
      ...
      expect(agents).toContain('## Filler 44');
    });
  }
});
