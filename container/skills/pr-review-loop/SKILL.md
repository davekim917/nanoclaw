...
codex-review.sh receipt --head <sha> --outcome approve|changes --reviewer "<model + runtime>" --body-file <file>
                                         ...
                                         # must name a model in container/skills/pr-review-loop/reviewer-models.txt
```
...
1. After opening the PR, run `codex-review.sh scope`. It waits for the `Risk label` run on the current head (`CODEX_REVIEW_SCOPE_TIMEOUT_SECONDS`, default 300) and prints a `verdict`. It fails closed: a run that is missing, late, failed, or only on an older head gives `review`, never `skip`. So does a `risk:high` or `review:requested` label that anyone but the labeler (`github-actions[bot]`) took off, one that is gone with no `unlabeled` event (the label was deleted or renamed), and label events it cannot read.
...
4. Merge only the head `merge-check` evaluated, within Step 6's authorization rule:
   ...
   not a single-file extraction of `codex-review.sh` — it reads
   `reviewer-models.txt` next to itself, and a copy with no sibling file fails
   closed (every receipt is refused as unlisted) rather than silently skipping
   the check. Running it from inside a checkout of main (the normal case) needs
   nothing extra. Driving it from a scratch extraction instead — e.g. from
   inside the PR's own worktree, or any context that doesn't already have
   main's tree on disk — extract the directory, not the file:

   ```bash
   git archive origin/main container/skills/pr-review-loop | tar -x -C <dir>
   <dir>/container/skills/pr-review-loop/scripts/codex-review.sh merge-check --head "$SHA"
   ```
