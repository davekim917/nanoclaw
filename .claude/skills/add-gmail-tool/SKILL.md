...
Pinned version matters — the latest-stable policy still requires a fixed ARG version for every Node CLI installed into the image, with the release adapter and container tests reviewing any bump.
