...
| P3 | E3 — free-text search | Search matches `series_id / agent_group_name / channel_name / cron` (the fields present on snapshot rows), not the prompt body. The plan E3 prose says "search over prompt," but prompt text is **deliberately excluded from the lean list snapshot** (fetched only in the detail drawer, per the read-layer design D-decision); searching prompt would require widening the snapshot the design rejects. | **ACCEPT as plan-vs-design tension** — impl faithfully follows the design. Flagged to Operator at the ship gate as a scope note (prompt-text search is not in v1; the prompt is visible per-task in the drawer). |
